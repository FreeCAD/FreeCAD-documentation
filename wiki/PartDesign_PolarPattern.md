...
-   Specify the **Angle** to be covered by the pattern. If the angle is less than 360°, the instances are evenly distributed from 0° (first instance) to the given angle (last instance). If the angle is a full 360°circle, then the instances are distributed evenly around the circle. That means for *n* instances an angle of 360° is equivalent to an angle of 360°\*(1-1/*n*).
...
---
![](images/Right_arrow.png) [documentation index](../README.md) > [PartDesign](PartDesign_Workbench.md) > PartDesign PolarPattern
