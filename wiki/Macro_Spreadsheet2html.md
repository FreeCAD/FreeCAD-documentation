...
<head><meta http-equiv="content-type" content="text/html; charset=utf-8"/>
    ...
    <meta name="created" content=""/><style type="text/css">
...
if options.get('CommentTable'):
    ...
    """
    table_extra = ET.fromstring(template_extra)table_extra.set('border', str(int(options.get('TableBorder'))))body = doc.find('./body')
    body.extend((ET.Element('p'), table_extra))for address, attrib in cells.items():
        ...
        if content.startswith('='):
            value = spreadsheet.get(address)
            row[2] = value
            row[4] = chr(39) + content    tr = ET.Element('tr')
        for r in row:
            td = ET.Element('td')
            td.text = r
            tr.append(td)
...
---
[documentation index](../README.md) > Macro Spreadsheet2html
