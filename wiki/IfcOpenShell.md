...
If the directory is correctly moved, test that the  
```python
>>> import ifcopenshell
...
If there were no error messages during configuration with CMake, a  
```python
make -j N
```
...
---
![](images/Button_right.svg) [documentation index](../README.md) > [BIM](Category_BIM.md) > [3rd Party](Category_3rd Party.md) > [Arch](Category_Arch.md) > [FEM](Category_FEM.md) > IfcOpenShell
