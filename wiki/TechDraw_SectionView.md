...
-   **Section Line Format**: two section line formats are supported (as depicted above) and controlled by the Preference setting \"Section Line Standard\" on the Annotation tab. Option {{Value|ANSI}} uses \"pulling arrows\" (known as the \"traditional format\" in some areas) and option {{Value|ISO}} uses \"pushing arrows\" (also known as the reference arrow format).
