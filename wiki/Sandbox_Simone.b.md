...
This macro creates an [Hilbert curve](https://en.wikipedia.org/wiki/Hilbert_curve) wire in 2 or 3 dimensions with many iterations.
...
-   Select if the curve lay on a plane {{CheckBox|TRUE|2D}} or is in the space {{CheckBox|FALSE|3D}}.
-   Insert the number of the iterations {{SpinBox|1}}. *Attention!* Increasing the number of iterations increases the computation time.
-   Insert length of the wire segment {{SpinBox|10.00}}.
...
If you clicked **OK**, you can use the just created wire to do a sweep in the part Workbench, but first of all it\'s better to apply a radius to the wire, or the sweep will be badly formed.
...
---
![](images/Right_arrow.png) [documentation index](../README.md) > Sandbox:Simone.b
