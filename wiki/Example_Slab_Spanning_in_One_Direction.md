...
*A Example of Slab Reinforcement of spanning in one direction in Slab [Arch Structure](Arch_Structure.md)*
...
7. Now select BentShapeRebar rebar type and other desired data for input data for rabars in cross direction of selected face as show in below image.
...
-   Creates a `SlabReinforcementGroup` object for slab spanning in one direction from the given `structure`, which is a Slab [Arch Structure](Arch_Structure.md), and `facename`, which is a face of that structure.
...
---
[documentation index](../README.md) > [Reinforcement](Category_Reinforcement.md) > Example Slab Spanning in One Direction
