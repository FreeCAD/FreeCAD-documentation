---
 GuiCommand:
   Name: TechDraw ExtensionInsertSquare
   MenuLocation: TechDraw , Extensions: Dimensions , Insert '□' Prefix
   Workbenches: TechDraw_Workbench
   Shortcut: 
   Version: 0.20
   SeeAlso: TechDraw_ExtensionInsertDiameter, TechDraw_ExtensionInsertRepetition
---

# TechDraw ExtensionInsertSquare

## Description

The **TechDraw ExtensionInsertSquare** tool inserts a \'□\' symbol at the beginning of the dimension text.

 <img alt="" src=images/TechDraw_extensionInsertSquareExample.png  style="width:500px;">  
*On the right a dimension with the inserted symbol*

## Usage

1.  Select one or more dimensions.
2.  There are several ways to invoke the tool:
    -   Press the **<img src="images/TechDraw_ExtensionInsertSquare.svg" width=16px> [Insert '□' Prefix](TechDraw_ExtensionInsertSquare.md)** button.
    -   Select the **TechDraw → Extensions: Dimensions → <img src="images/TechDraw_ExtensionInsertSquare.svg" width=16px> Insert '□' Prefix** option from the menu.
3.  The \'□\' symbol is inserted in the dimension text of the selected dimensions.




 {{TechDraw_Tools_navi}}



---
⏵ [documentation index](../README.md) > [TechDraw](TechDraw_Workbench.md) > TechDraw ExtensionInsertSquare
