...
The output shape occupies the same space as the original. But it is split where it intersects with other shapes. The split pieces are put into a compound (or compsolid), so the object appears to remain in one piece. You need to explode the compound to get the individual pieces. If you want to access the individual pieces in a parametric way you can use <img alt="" src=images/Part_CompoundFilter.svg  style="width:24px;"> _.
...
---
[documentation index](../README.md) > [Part](Part_Workbench.md) > Part Slice
