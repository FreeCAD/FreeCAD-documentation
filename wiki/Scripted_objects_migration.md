...
Now we consider that the workbench is restructured, so that classes aren\'t just at the root directory, but instead are inside an  
```python
...
In this case, we assume that we have also defined a new [viewprovider](viewprovider.md) in the module  
```python
# old_module.py
import FreeCAD as App
import objects.new_module as new_module
...
This attribute is inspected by looking at the  
```python
>>> obj = App.ActiveDocument.Custom
>>> print(obj.Proxy.ver)
0.18
