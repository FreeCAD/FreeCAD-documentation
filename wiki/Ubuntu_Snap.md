...
Starting from the tail end of the v0.20dev release cycle, the FreeCAD snap maintainers added the ability to test experimental FreeCAD builds. Snaps allow for this by easily toggling between different snaps (terminology is \'[channels or tracks](https   *//snapcraft.io/docs/channels)\'). For example   *
...
Using the  {{code|lang=bash|code=
snap refresh freecad --channel=latest/edge/toponaming
}}

Toggling back to the nightly \'edge\' channel   *  {{code|lang=bash|code=
