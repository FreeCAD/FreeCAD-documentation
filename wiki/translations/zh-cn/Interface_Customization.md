...
由于FreeCAD界面是基于现代的[Qt](http://en.wikipedia.org/wiki/Qt_(toolkit))工具包制作，因此它具有最先进的组织架构。widget部件、菜单、工具栏与其他工具都可修改、移动、在工具台之间共享，还可设置、修改快捷键，并记录和执行宏（macro）。通过{{MenuCommand|Tools → Customize}}菜单即可进行窗口的定制：
