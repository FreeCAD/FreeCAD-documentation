...
1.  **Language**: （*通用*类别，*通用*选项卡）FreeCAD 将自动选择与您操作系统语言相对应的语言，但您可能希望进行更改。FreeCAD 几乎完全翻译成五到六种语言；其他语言目前只有部分翻译。您可以轻松地[帮助翻译 FreeCAD](https://crowdin.com/project/freecad)。![](images/Freecad-basic-options01.jpg )
2.  **Auto-load module**: （*通用*类别，*通用*选项卡）通常，FreeCAD 将显示启动页面。您可以跳过此步骤，直接在您选择的工作台中开始 FreeCAD 会话，这些选项列在 *启动*，*启动后自动加载模块* 下。[工作台](工作台.md) 将在 [下一章节](手册：FreeCAD_界面.md) 中详细解释。
3.  **Create new document at startup**: （*通用*类别，*文档*选项卡）与上述 *自动加载模块* 选项结合使用，如果选中此项，将直接启动准备工作就绪的 FreeCAD。![](images/Freecad-basic-options02.jpg )
4.  **Storage options**: （*通用*类别，*文档*选项卡）与任何复杂应用程序一样，FreeCAD 可能存在导致偶尔崩溃的错误。在此您可以配置选项以在崩溃时帮助恢复工作。
5.  **Authoring and license**: （*通用*类别，*文档*选项卡）在此设置新创建文件的值。考虑从一开始就使用友好的 [https://en.wikipedia.org/wiki/Copyleft Copyleft](https://en.wikipedia.org/wiki/Copyleft_Copyleft.md) 许可证（如 [https://creativecommons.org/ Creative Commons](https://creativecommons.org/_Creative_Commons.md)）使您的文件可共享。
6.  **Redirect internal python messages**: （*通用*类别，*输出窗口*选项卡）这两个选项总是很好的选择，因为它们将导致内部 Python 解释器的消息在 [报告视图](手册：FreeCAD_界面＃报告视图.md) 中显示出问题运行 Python 脚本时。![](images/Freecad-basic-options03.jpg )
7.  **Units**: （*通用*类别，*单位*选项卡）在这里，您可以设置您希望使用的默认单位系统。![](images/Freecad-basic-options04.jpg )
8.  **Zoom at cursor**: （*显示*类别，*3D*选项卡）如果设置，缩放操作将以鼠标指针为焦点。如果取消设置，则当前视图的中心是缩放焦点。
9.  **Invert zoom**: （*显示*类别，*3D*选项卡）反转与鼠标移动相关的缩放方向。![](images/FreeCAD-v0-18-Preferences-Display.png )
...
---
![](images/Button_right.svg) [documentation index](../README.md) > [Poweruser Documentation](Category_Poweruser Documentation.md) > [Tutorials](Category_Tutorials.md) > Manual:Installing/zh-cn
