...
FreeCAD 跨平台可用，（在Windows、Mac OS 和 Linux 平台上运行方式完全相同），它是 [open source\|开源](http://en.wikipedia.org/wiki/Open-source_software) 的。作为开源软件，FreeCAD 受益于全球范围内大量的程序员、爱好者和用户的贡献和努力。FreeCAD 基本上是由使用它的人构建的应用程序，而不是由一家试图向您销售产品的公司制作的。这也意味着 FreeCAD 不仅免费使用，而且可以自由分发、复制、修改，甚至销售。
