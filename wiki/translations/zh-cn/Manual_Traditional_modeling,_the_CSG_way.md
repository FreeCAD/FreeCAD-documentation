...
虽然其他工作台通常提供更高级的工具来构建和操作几何图形，但由于它们实际上都是操作零件对象，因此了解这些对象在内部是如何工作的，能够使用 Part 工具非常有用。由于 Part 工具更简单，它们经常可以帮助您解决更智能的工具无法正确解决的问题。
...
请注意，尽管桌腿厚度为 8 毫米，但我们将其放置在距离 10 毫米处，留下 2 毫米的间隙。当然，这并不是必要的，它在真实的桌子上不会发生，但在这种"组装"模型中，这是一种常见的做法，它可以帮助观察模型的人理解这些是独立的部件，需要稍后手动连接在一起。
...
-   **顶点**: 这些是构建所有其他几何体的点（通常是端点）。例如，一条线有两个顶点。
-   **边**: 边是线性几何体，例如线段、弧线、椭圆或[NURBS](https://en.wikipedia.org/wiki/Non-uniform_rational_B-spline) 曲线。它们通常有两个顶点，但某些特殊情况只有一个（例如封闭圆）。
-   **线**: 线圈是由它们的端点连接的一系列边。它可以包含任何类型的边，并且可以是封闭的或非封闭的。
-   **面**: 面可以是平面或曲面，并且可以由一个封闭线圈形成，该线圈形成面的边界，或者由多个线圈形成，比如面里有孔。
-   **壳**: 壳只是由它们的边缘连接的一组面。它可以是开放的或封闭的。
-   **实体**: 当壳紧密封闭，即没有"泄漏"时，它就成为实体。实体具有内部和外部的概念。许多工作台依赖于此，以确保它们生成的对象可以在现实世界中构建。
-   **复合体**: 复合体只是其他几何体的聚合体，无论它们的类型如何，都聚合为一个单一的几何体。

在 3D 视图中，您可以选择单个的**顶点**、**边**或**面**。选择其中之一也会选择整个对象。
