...
利用[零件工作台（Part Workbench）](Part_Workbench.md)中的立方体命令可向处于活动状态的文档中插入一个参数化 [长方体](http://en.wikipedia.org/wiki/Cuboid#Rectangular_cuboid)几何图元。默认情况下，此立方体命令会在原点处插入一个10x10x10 mm且附有\"cube\"标签的立方体。在添加立方体对象后还可以修改这些参数。
...
通过下列函数即可在[宏](macros.md)以及python控制台中使用立方体命令：
