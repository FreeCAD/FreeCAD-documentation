# Which workbench should I choose/it
Se si è nuovi in FreeCAD, è molto probabile che ci si stia chiedendo con quale [Ambiente di lavoro](Workbenches/it.md) si debba iniziare a lavorare. Questa pagina aiuterà a scegliere da dove iniziare.
...
FreeCAD viene fornito con diversi [ambienti di lavoro integrati](Workbenches/it.md), ma ne sono disponibili [molti altri](External_workbenches/it.md) e possono essere facilmente installati tramite il [gestore dei componenti aggiuntivi](Std_AddonMgr/it.md).

I nuovi utenti di FreeCAD generalmente iniziano a utilizzare e ad apprendere uno o due ambienti di lavoro specifici, quindi esplorano altre aree di FreeCAD e aggiungono gli strumenti che trovano interessanti per le loro competenze. Prima di immergersi in ambienti di lavoro specifici, tuttavia, assicurarsi di leggere le pagine [Iniziare](Getting_started/it.md) e [Navigazione nello spazio 3D](Manual:Navigating_in_the_3D_view/it.md), poiché forniscono conoscenze generiche di cui si avrà bisogno ovunque in FreeCAD. Il [Manuale di FreeCAD](Manual:Introduction/it.md) è un altro buon metodo per scoprire FreeCAD passo dopo passo, in modo lineare.
...
Questo è probabilmente il caso più comune tra i nuovi utenti di FreeCAD ed è quello migliore in FreeCAD. È dotato di un ambiente di lavoro specifico per questo caso: l\'ambiente [PartDesign](PartDesign_Workbench/it.md). PartDesign contiene anche tutti gli strumenti di [Sketcher](Sketcher_Workbench/it.md), quindi si imparano e utilizzano due ambienti di lavoro in uno.

Quando si inizia un nuovo modello con PartDesign, in genere verrà prima richiesto di creare un [Corpo](PartDesign_Body/it.md). Un corpo è allo stesso tempo un contenitore per sottoforme e il risultato, l\'oggetto finale. Va pensato come un oggetto = un corpo. Il corpo, anche se può essere composto da più parti, deve sempre rappresentare un unico oggetto assemblato, senza parti separate. La maggior parte delle operazioni eseguite su o all\'interno di un corpo impediranno di creare parti separate.
...
---
⏵ [documentation index](../README.md) > Which workbench should I choose/it
