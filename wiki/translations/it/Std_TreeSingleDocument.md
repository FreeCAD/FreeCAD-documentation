...
Nella modalità Documento singolo è possibile passare a un altro documento attivando una vista 3D appartenente a quel documento. Si attivarlo tramite l\'[area di visualizzazione principale](Main_view_area/it.md) o tramite il menu {{MenuCommand|Finestre}}.

## Utilizzo

1.  There are several ways to invoke the command:
    -   Click on the black down arrow to the right of the **<img src="images/Std_TreeSyncView.svg" width=16px>** button and select the {{MenuCommand|Single document}} option from the flyout. Note: the button image will change depending on the selected option.
    -   Select the {{MenuCommand|View → TreeView actions → <img src="images/Std_TreeSingleDocument.svg" width=16px> Single document}} option from the menu.
