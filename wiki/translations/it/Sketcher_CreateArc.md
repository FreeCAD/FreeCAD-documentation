---
 GuiCommand:
   Name: Sketcher CreateArc
   Name/it: Arco
   MenuLocation: Sketch , Geometrie , Arco da centro
   Workbenches: Sketcher_Workbench/it
   Shortcut: **G** **A**
   SeeAlso: Sketcher_CompCreateCircle/it
---

# Sketcher CreateArc/it



## Descrizione

Disegna un arco dalla selezione di tre punti: il **centro**, l**\'angolo iniziale** lungo (contemporaneamente) il **raggio**, e l**\'angolo finale**.

Quando si avvia lo strumento, il puntatore del mouse assume la forma di croce bianca accompagnata dall\'icona rossa di un arco. Accanto al puntatore sono visualizzate, in blu, le coordinate della sua posizione aggiornate in tempo reale.

<img alt="Il centro è stato selezionato, spostare il mouse per regolare il raggio" src=images/Sketcher_ArcExample1.png ) ![Il centro e l\'angolo di partenza sono stati selezionati, spostare il mouse per regolare l\'angolo finale](images/Sketcher_ArcExample2.png  style="width:407" height="360px;">



## Utilizzo

-   Seleziona punti su un\'area vuota della vista 3D o su un oggetto esistente (i vincoli automatici devono essere attivi nel [Pannello delle azioni](Task_panel/it.md)).
-   Premere il tasto **Esc**, o cliccare sul tasto destro del mouse, per annullare l\'operazione.





{{Sketcher_Tools_navi

}}



---
⏵ [documentation index](../README.md) > [Sketcher](Sketcher_Workbench.md) > Sketcher CreateArc/it
