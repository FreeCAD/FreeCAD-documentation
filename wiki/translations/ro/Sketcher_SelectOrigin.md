# Sketcher SelectOrigin/ro
---
- GuiCommand:   Name:Sketcher SelectOrigin   Workbenches:[[Sketcher Workbench   Sketcher]]|MenuLocation:Sketch → Sketcher tools → Select Origin   Shortcut:Shift+Ctrl+O   Version:0.15---


</div>



## Descriere

Selectează originea schiței.




<div class="mw-translate-fuzzy">

## Utilizare


</div>


<div class="mw-translate-fuzzy">

1.  Intrați în modul edit al schiței.
2.  Alegeți traseul ** Sketch** → ** Sketcher tools** → **<img src="images/Sketcher_SelectOrigin.png" width=32px> Select Origin** din meniul principal.
3.  Originea schiței va fi selectată.


</div>





{{Sketcher_Tools_navi

}}



---
![](images/Right_arrow.png) [documentation index](../README.md) > [Sketcher](Sketcher_Workbench.md) > Sketcher SelectOrigin/ro
