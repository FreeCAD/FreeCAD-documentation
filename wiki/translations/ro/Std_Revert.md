---
- GuiCommand:/ro   Name:Std Revert   Name/ro:Restabilire   MenuLocation:[[Std_File_Menu/ro   File]] → Restabilire|Workbenches:All   Shortcut:   SeeAlso:---


</div>

## Description

The **Std Revert** command closes the active document and reopens the last saved version of the document.

## Usage

1.  Select the {{MenuCommand|File → <img src="images/Std_Revert.svg" width=16px> Revert}} option from the menu.
2.  Press the **Yes** button in the dialog box to confirm you want to lose all changes since the last file save.

## Options

-   Press **Esc** or the **No** button to abort the command.





{{Std Base navi

}}  
