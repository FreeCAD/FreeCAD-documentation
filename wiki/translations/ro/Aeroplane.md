...
-   Crează un cilindru făcând clic pe <img alt="" src=images/Part_Cylinder.png  style="width:32px;"> [Cylinder](Part_Cylinder.md) button.
...
We are now going to merge these two shapes together with a Boolean Operation. Click on the <img alt="" src=images/Part_Boolean.svg  style="width:32px;"> [Booleas](Part_Boolean.md) button and the Tasks viewer will display the Boolean Operation selector.
...
Click on Box and get its [Placement into Tasks](Tasks_Placement.md). Notice it already has Y: -10 and Z: -1 in the Translation. Change the X translation to 3 and click on Apply. That\'s better. Click OK.
...
Faceți clic pe Aeroplane și obțineți-l [Placement into Tasks](Tasks_Placement.md) (Other explanation on [Placement](Placement.md)). În schimbarea secțiunii de rotație, unde se afișează \"Axa de rotație cu unghi\" cu \"Unghiurile Euler\", pentru că sunt mult mai ușor de utilizat.
...
-   Although the three labels are Yaw, Pitch and Roll that\'s not really what they are. Yaw, Pitch and Roll are references to the *body coordinates* of an object in 3D space. The labels should be Heading, Elevation and Bank or even Azimuth, Inclination and Bank because thay actually refer to the *space coordinates* of the 3D system. These are the **Tait-Bryan angles**. If you want more information then try [Euler Angles](http://en.wikipedia.org/wiki/Euler_angles#Tait-Bryan_angles).
...
---
⏵ [documentation index](../README.md) > [Part](Category_Part.md) > Aeroplane/ro
