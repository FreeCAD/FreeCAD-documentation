...
Această pagină colectează toate informațiile despre sistemul de rezolvare FEM utilizat de [FEM Workbench](FEM_Workbench.md) al FreeCAD. Interfața dintre un solver și FreeCAD în pre- și post-procesare se face prin fișiere text. Acest lucru înseamnă teoretic orice solver care utilizează fișiere text este capabil să lucreze împreună cu FreeCAD FEM.
...
## Versiunea Solver în diferite distribuții Linux {#versiunea_solver_în_diferite_distribuții_linux}
...
## Solvers with an interface in FreeCAD {#solvers_with_an_interface_in_freecad}
...
## Solvers implemented as external workbenches {#solvers_implemented_as_external_workbenches}
...
### FastHenry and FasterCap {#fasthenry_and_fastercap}
