...
Зависимости должны быть установлены в системе перед компиляцией; см. [ компиляция в Linux](Compile_on_Linux/ru.md), [ компиляция в Windows](Compile_on_Windows/ru.md) и [ компиляция в macOS](Compile_on_MacOS/ru.md) для получения большей информации.
...
-   для записи [macros](macros.md) и записи макросов,
-   для реализации логики приложений для стандартных пакетов,
-   для реализации вспомогательных инструментов, таких как [Addon Manager](Addon_Manager.md),
-   для реализации целых рабочих столов, таких как [Draft](Draft_Workbench.md) и [Arch](Arch_Workbench.md),
...
### Технология OpenCascade {#технология_opencascade}
...
#### Shiboken2 и Pyside2 {#shiboken2_и_pyside2}
...
Coin3D переносим на широкий спектр платформ: операционные системы UNIX, Linux, BSD, macOS X и Microsoft Windows. Чтобы узнать больше об этой библиотеке, посетите [Coin3D homepage](https://github.com/coin3d/coin).

#### SoQt (устарело) {#soqt_устарело}
...
[Pivy](Pivy.md) - это библиотека, которая завёртывает библиотеку Coin3d для использования в [Python](Python.md). Он не нужен для сборки FreeCAD или для его запуска, но он необходим в качестве зависимости во время выполнения [Draft Workbench](Draft_Workbench.md) и другими инструментальными средствами, которые используют его внутри, например [Arch](Arch_Workbench.md) и [BIM](BIM_Workbench.md).
...
Ply-это синтаксический анализатор Python-Lex-Yacc. Он используется в качестве зависимости во время выполнения [OpenSCAD Workbench](OpenSCAD_Workbench.md). Если вы не используете этот инструментальное средство, вам может не понадобиться этот пакет.
...
Zipios++ - это библиотека C++ для чтения и записи файлов {{incode/.zip}}. Доступ к отдельным записям обеспечивается с помощью стандартных потоков ввода-вывода C++. Простая виртуальная файловая система, доступная только для чтения, которая монтирует обычные каталоги и {{incode/.zip}} файлы также предоставляются. Структура и общедоступный интерфейс Zipios++ в значительной степени основаны на пакете Java {{incode/java.util.zip}}.

Собственный формат файла FreeCAD `.FCstd` на самом деле является {{incode/.zip}} файл, в котором хранятся и сжимаются другие типы данных внутри него, такие как файлы BREP и XML. Поэтому Zipios++ используется для сохранения и открытия сжатых архивов, включая файлы FreeCAD.
...
Zlib разработан как бесплатная универсальная библиотека сжатия данных без потерь для использования на любом компьютерном оборудовании и операционной системе. Он реализует алгоритм сжатия `DEFLATE` , обычно используемый в {{incode/.zip}} и `.gzip` файлы.
...
Копия библиотеки включена в исходный код [Path Workbench](Path_Workbench.md), поэтому она компилируется вместе с ним.
...
Если вы работаете под Linux, вам не нужен пакет LibPack, так как вы можете получить зависимости из репозиториев вашего дистрибутива, как указано на странице [компиляция в Linux](Compile_on_Linux/ru.md).
