---
- GuiCommand:/ru
   Name:Sketcher CreateArcOfHyperbola
   Name/ru:Sketcher Дуга по гиперболе
   MenuLocation:Эскиз → Геометрия эскиза → Дуга по гиперболе
   Workbenches:[Sketcher](Sketcher_Workbench/ru.md)
   Version:0.17
   SeeAlso:[Sketcher Дуга по эллипсу](Sketcher_CreateArcOfEllipse.md), [Sketcher Дуга по параболе](Sketcher_CreateArcOfParabola.md)
---

## Описание

Этот инструмент рисует дугу по гиперболе используя центр, большой радиус и конечные точки.

![](images/Sketcher_Arc_of_Hyperbola_01.png )


<div class="mw-translate-fuzzy">

## Как использовать {#как_использовать}


</div>


<div class="mw-translate-fuzzy">

1.  Нажмите кнопку **<img src=images/Sketcher_Hyperbolic_Arc.png style="width:24px"> '''Дуга по гиперболе'''**.
2.  Нажмите в 3D виде, чтобы задать центр большого радиуса (обозначен 1 на рисунке выше).
3.  Нажмите в 3D виде, чтобы задать большой радиус, который также является вершиной дуги гиперболы (обозначена 2 на рисунке выше).
4.  Двигайте мышь от двух последних выбранных точек, чтобы увидеть, как будет выглядеть дуга. Нажмите еще раз, когда будет получена приблизительная желаемая кривизна. Эта точка будет первой конечной точкой (обозначена 3 на рисунке выше).
5.  Щелкните четвертый раз в 3D виде, чтобы задать другую конечную точку (обозначена 4 на рисунке выше).


</div>





{{Sketcher Tools navi

}}  
