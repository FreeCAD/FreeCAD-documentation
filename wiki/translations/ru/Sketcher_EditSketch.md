---
- GuiCommand:/ru
   Name:Sketcher EditSketch
   Name/ru:Редактировать эскиз
   MenuLocation:Sketch → Редактировать эскиз
   Workbenches:[Sketcher](Sketcher_Workbench/ru.md), [PartDesign](PartDesign_Workbench/ru.md)
   SeeAlso:[Sketcher LeaveSketch](Sketcher_LeaveSketch/ru.md)
---

## Описание


<div class="mw-translate-fuzzy">

Эта команда позволяет редактировать существующий эскиз. Она открывает [Sketcher Dialog](Sketcher_Dialog/ru.md).


</div>

## Использование


<div class="mw-translate-fuzzy">

Есть несколько способов вызвать команду Edit Sketch:

-   В [древе проекта](tree_view/ru.md) нажмите на эскизе правую кнопку мыши и выберите *Редактировать эскиз*
-   Выделите эскиз в [древе проекта](tree_view/ru.md) и в выпадающем меню выберите {{MenuCommand|Sketch → Edit Sketch}} *Sketch → Редактировать эскиз*.
-   Просто дважды щелкнув левой кнопкой мыши на эскизе.
-   Кликнув на кнопке <img alt="" src=images/Sketcher_EditSketch.svg  style="width:24px;"> панели инструментов Sketcher.


</div>

## Примечания


<div class="mw-translate-fuzzy">

-   Выбор [Leave Sketch](Sketcher_LeaveSketch/ru.md) закончит режим редактирования.


</div>





{{Sketcher Tools navi

}}  
