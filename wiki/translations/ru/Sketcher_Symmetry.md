---
- GuiCommand:/ru
   Name:Sketcher Symmetry
   Name/ru:Sketcher Symmetry
   MenuLocation:Sketch → Инструменты для эскиза → Симметрия
   Workbenches:[Sketcher](Sketcher_Workbench/ru.md)
   Version:0.16
---


</div>

## Description

Mirrors sketcher geometry in reference to a chosen line or sketch axis.

## Usage

1.  Select the geometry to be copied, then a line or sketch axis to be used as the symmetry axis.
2.  Press the **<img src=images/Sketcher_Symmetry.svg style="width:16px"> <img src=images/Sketcher_Symmetry.svg style="width:Symmetry](Sketcher_Symmetry.md)** button or choose {{MenuCommand|Sketch → Sketcher tools → [16px"> Symmetry}} from the top menu.
3.  The selected geometry will be copied symmetrically to the selected line or sketch axis.

Note that no extra symmetry constraint will be added.





{{Sketcher Tools navi

}}  
