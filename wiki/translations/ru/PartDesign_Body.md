...
[Тело](PartDesign_Body/ru.md) является базовым элементом для создания деталей с помощью [верстака PartDesign](PartDesign_Workbench/ru.md). Оно может включать в себя [эскизы](Sketch/ru.md), [опорные объекты](Datum/ru.md), и [конструктивные элементы](PartDesign_Feature/ru.md) которые помогают в построении [сплошных твердых тел](PartDesign_Body/ru#Single_contiguous_solid.md).

Тело предоставляет объект с **системой координат** которая включает в себя локальные оси X, Y и Z, а также стандартные плоскости. Эти элементы Тела можно использовать для расположения [эскизов](Sketch/ru.md) и [простых геометрических форм](PartDesign_CompPrimitiveAdditive/ru.md).

Не путайте <img alt="" src=images/PartDesign_Body.svg  style="width:24px;"> [Тело PartDesign](PartDesign_Body/ru.md) с <img alt="" src=images/Std_Part.svg  style="width:24px;"> [Деталью Std](Std_Part/ru.md). Первый --- это специальный объект, используемый в <img alt="" src=images/Workbench_PartDesign.svg  style="width:24px;"> [Верстаке PartDesign](PartDesign_Workbench/ru.md), предназначенный для моделирования [единого непрерывного твёрдого тела](PartDesign_Body/ru#Single_contiguous_solid.md) с помощью [Конструктивных элементов PartDesign](PartDesign_Feature/ru.md). [Деталь Std](Std_Part/ru.md) это группирующий объект, предназначенный для создания [сборок](assembly/ru.md); он не используется для моделирования, просто для размещения различных объектов в пространстве. Для создания сложной сборки в одну [Деталь Std](Std_Part/ru.md) можно поместить несколько тел и другие [Детали Std](Std_Part/ru.md).

![](images/PartDesign_Body_tree.png ) ![](images/PartDesign_Body_example.png ) 
*Слева: древовидный вид, содержащий конструктивные элементы, которые последовательно формируют форму объекта. Справа: конечный объект полученный в результате совокупности всех этих операций, отображаемый в окне [трехмерного Вида](3D_view/ru.md).*
...
Открытый документ может содержать в себе сразу несколько тел. Чтобы добавить новый конструктивный элемент к некоторому Телу, его необходимо сделать **активным**. Активное тело будет подсвечено в [иерархии документа](tree_view/ru.md) цветом фона, заданным **Active container** значением в [редакторе настроек](Preferences_Editor#Colors/ru.md) (по умолчанию светло-синий). Активное тело также будет выделено жирным шрифтом.

Чтобы активировать или деактивировать Тело:

-   Дважды щелкните по нему в [древе проекта](tree_view/ru.md), или
...
---
![](images/Right_arrow.png) [documentation index](../README.md) > [PartDesign](PartDesign_Workbench.md) > PartDesign Body/ru
