---
- GuiCommand:/ru   Name:Arch ToggleSubs   Name/ru:Arch ToggleSubs   Workbenches:[[Arch Module/ru   Arch]]|MenuLocation:Архитектура → Утилиты → Toggle Subcomponents   Shortcut:‏‎   SeeAlso:---


</div>

## Описание


<div class="mw-translate-fuzzy">

Этот инструмент переключает видимость всех вычитаний объекта [Arch](Arch_Workbench.md) между видимыми и скрытыми. Если объект Arch содержит некоторые видимые и некоторые скрытые вычитаемые объекты, все они станут видимыми / скрытыми после запуска этой команды.


</div>

Normally, if an Arch object, like an [Arch Wall](Arch_Wall.md), is selected and you press **Space** only the external wall will be hidden or made visible, but not the internal objects.

With this tool, the internal subtracted objects will all become visible or hidden.


<div class="mw-translate-fuzzy">

## Использование


</div>


<div class="mw-translate-fuzzy">

1.  Выберите объект Arch с вычитаниями
2.  Выберите меню Arch → Utilities → **<img src="images/Arch_ToggleSubs‏‎.png" width=16px> [Toggle Subcomponents‏‎](Arch_ToggleSubs‏‎.md)
**


</div>

## Опции

The color of a subtraction element can follow the style of [Draft construction](Draft_ToggleConstructionMode.md) elements, by activating the appropriate option in the [Arch Preferences](Arch_Preferences.md), in the menu {{MenuCommand|Edit → Preferences → Arch → General settings → Apply Draft construction style to subcomponents}}.

The [Draft construction](Draft_ToggleConstructionMode.md) style can be set in the [Draft Preferences](Draft_Preferences.md), in the menu {{MenuCommand|Preferences → Draft → General Draft}}.





 
