...
На этой странице собрана вся информация о решателях метода конечных элементов (МКЭ), используемых [верстаком FEM](FEM_Module/ru.md). Интерфейс между решателем и FreeCAD в предварительной и последующей обработке выполнен через текстовые файлы. Теоретически это означает, что любой решатель, который может быть сконфигурирован и управляться через текстовые файлы, способен работать с FreeCAD, чтобы эта коммуникация действовала, необходимо запрограммировать соответствующий синтаксический анализатор и писатель входных и выходных файлов.
...
### Available solvers in various Linux distributions {#available_solvers_in_various_linux_distributions}
