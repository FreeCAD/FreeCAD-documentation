...
-    **_; снова используйте эту команду, чтобы отменить её действие. Она устанавливает атрибут `Selectable` в значение `True` или `False`. Измените свойство {{PropertyView/ru|Selectable}} переключением значения в [редакторе(окне) свойств](Property_editor/ru.md).
