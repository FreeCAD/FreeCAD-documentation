...
-    **[Видимость](Std_ToggleVisibility/ru.md)**: делает объект видимым или невидимым в [3D-виде](3D_view/ru.md).

-    **[Показать выделенные](Std_ShowSelection/ru.md)**: делает выбранные объекты видимыми.

-    **[Скрыть выделенные](Std_HideSelection/ru.md)**: делает выбранные объекты невидимыми.

-    **[Откл/вкл выделяемость](Std_ToggleSelectability/ru.md)**: делает объект больше не выбираемым в [3D-виде](3D_view/ru.md); снова используйте эту команду, чтобы отменить её действие. Она устанавливает атрибут `Selectable` в значение `True` или `False`. Измените свойство {{PropertyView/ru|Selectable}} переключением значения в [редакторе(окне) свойств](Property_editor/ru.md).

-    **[Выбрать все экземпляры](Std_TreeSelectAllInstances/ru.md)**: выбирает все экземпляры этого объекта в древе проекта.
...
-    **[Случайный цвет](Std_RandomColor/ru.md)**: присваивает объекту случайный цвет. Он устанавливает атрибут `ShapeColor` в виде массива `(r,g,b)` с тремя случайными значениями в диапазоне от 0 до 1. Измените свойство, изменив {{PropertyView/ru|Shape Color}} в [редакторе свойств](Property_editor/ru.md).

-    **[Вырезать](Std_Cut/ru.md)**: неактивен если щелчок правой кнопкой мыши был сделан не на объекте.

-    **[Копировать](Std_Copy/ru.md)**: копирует объект в память.

-    **[Вставить](Std_Paste/ru.md)**: вставляет ранее скопированный объект в документ; копия объекта вставляется в конец древа проекта.

-    **[Удалить](Std_Delete/ru.md)**: удаляет объект из документа, и из древа проекта, путём вызова метода `removeObject()` документа.

-    **Utilities**: **(опционально)** дополнительные контекстные команды из [верстака Draft](Draft_Workbench/ru.md).
