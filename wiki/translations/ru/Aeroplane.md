...
## Первые шаги {#первые_шаги}

Мы будем работать в <img alt="" src=images/Workbench_Part.svg  style="width:24px;"> [верстаке Part](Part_Workbench/ru.md) - выберите его из меню через {{MenuCommand|Вид → Верстак → Part}} или [переключателем верстаков](Workbench_Selector/ru.md).
...
-   Убедитесь в том, что у вас отображается [Комбо-панель](Combo_View.md) (через {{MenuCommand|Вид → Панели}}).
...
Кликните [Placement (расположение)](Tasks_Placement/ru.md) (отметьте маленькую **[+]**), и появится кнопка с тремя точками**...**. Кликните на ней. (Вы можете так же выбрать: {{MenuCommand|Меню → Правка → Расположение}}.) Появится панель задач.
