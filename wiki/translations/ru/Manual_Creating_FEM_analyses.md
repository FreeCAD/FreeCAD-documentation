...
В FreeCAD такое моделирование сделано через _, и в конце калькуляция модели.
...
Моделирование производится сторонней программой, используемой FreeCAD для получения результатов. Поскольку симуляторов для моделирования по МКЭ с открытыми исходными кодами несколько, _.
...
Начнём с дома, который мы моделировали в разделе _. Нам потребуются некоторые изменения для приспособления модели к расчёту по МКЭ. В это входит исключение объектов, которые мы не хотим учитывать при вычислении, таких как двери и окна, и объединение всех остальных объектов в один.
...
-   Верстак FEM пока может вычислить деформацию лишь единого объекта. Поэтому нам надо объединить наши два объекта, стены и плиту. Переключимся на _. Теперь мы получили объединённый объект:
