# Robot SetDefaultValues/ru
---
- GuiCommand:/ru
   Name:Robot SetDefaultValues
   Name/ru:Robot SetDefaultValues
   Workbenches:[Robot](Robot_Workbench/ru.md)
   MenuLocation:Робот → Установка значений по умолчанию---


</div>


<div class="mw-translate-fuzzy">

#### Описание

Устанавливает значения по умолчанию для скорости, ускорения и целостности при создании промежуточных точек.


</div>

#### Использование

1.  Выберите в верхнем меню **Робот** → **<img src="images/Robot_SetDefaultValues.svg" width=32px>Установка значений по умолчанию**, чтобы установить:

:   

    :   \*скорость
    :   \*продолжительность (режим)
    :   \*ускорение


<div class="mw-translate-fuzzy">

#### Примечание

Программные значения по умолчанию следующие

-   скорость = 1 m/s
-   продолжительность = false
-   ускорение = 1 m/s\^2


</div>





{{Robot_Tools_navi

}}



---
![](images/Right_arrow.png) [documentation index](../README.md) > [Robot](Robot_Workbench.md) > Robot SetDefaultValues/ru
