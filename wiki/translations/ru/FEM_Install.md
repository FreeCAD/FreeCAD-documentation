...
Чтобы можно было проводить анализ методом конечных элементов (finite element analysis, FEA) с помощью **<img src="images/Workbench_FEM.svg" width=24px> [FEM Workbench](FEM_Workbench/ru.md)**, FreeCAD использует две внешние программы. Одна используется для генерации [сетки метода конечных элементов](FEM_Mesh/ru.md), а другой - для решения системы уравнений. Вы можете проверить, готова ли ваша установка FreeCAD настроен для выполнения механического анализа методом конечных элементов, загрузив и проанализировав пример [FEM CalculiX Cantilever 3D](FEM_CalculiX_Cantilever_3D/ru.md), включаемый в любую установку FreeCAD, начиная с версии 0.17.
...
In previous versions of FreeCAD, _ (the [Netgen button](FEM_MeshNetgenFromShape.md) was removed). Nevertheless, shortly afterwards some users reported success in patching Netgen 5.3.1, so that it worked with OCCT 7.x and FreeCAD.
...
The _ for more information on setting up the repositories.
