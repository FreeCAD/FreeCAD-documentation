---
- GuiCommand:/ru
   Name:Std ViewBoxZoom
   Name/ru:Std ViewBoxZoom
   Workbenches:All
   MenuLocation:Вид → Масштаб‏‎ → Увеличить область
   Shortcut:**Ctrl**+**B**
   SeeAlso:[Std ViewZoomIn](Std_ViewZoomIn/ru.md), [Std ViewZoomOut](Std_ViewZoomOut/ru.md)
---

## Описание

Команда **Std ViewBoxZoom** масштабирует вид в выделенной пользователем прямоугольной области до размеров всего активного [трёхмерного вида](3D_view.md).

## Применение

1.  Есть несколько способов вызвать команду:
    -   Выбрать в меню {{MenuCommand|Вид → Масштаб → <img src="images/Std_ViewBoxZoom.svg" width=16px> Увеличить область}}.
    -   Использовать клавиатурное сокращение: **Ctrl**+**B**.
2.  Выделить прямоугольник в окне трёхмерного вида.





{{Std Base navi

}}  
