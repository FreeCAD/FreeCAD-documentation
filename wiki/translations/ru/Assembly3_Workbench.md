# <img alt="иконка верстака Assembly3" src=images/Assembly3_workbench_icon.svg  style="width:64px;"> Assembly3 Workbench/ru
...
-   **заморозка сборки**. Поскольку ЦП может обрабатывать только определённое количество одновременных ограничений в реальном времени, замораживание сборки позволяет использовать ограничения даже для больших сборок. При замораживании готовых сборок или ограничений, которые не должны оставаться динамическими (например, сварные, болтовые или склеенные детали), они исключаются из расчётов обновления и считаются решателем Assembly3 фиксированной геометрией.
    ...
    :   Обратите внимание, что другие подходы предлагают другое решение этой проблемы, например <img alt="" src=images/Assembly4_workbench_icon.svg  style="width:24px;"> [Верстак Assembly4](Assembly4_Workbench/ru.md).
...
На Assembly3 повлиял [Assembly2](Assembly2_Workbench/ru.md), но они не совместимы. Если у Вас есть старые модели, созданные в Assembly2, оставайтесь на FreeCAD 0.16 и используйте Assembly2 с ним.
...
[Верстак Assembly3](Assembly3_Workbench/ru.md) (по состоянию на март 2022) доступен через [Менеджер Дополнений](Std_AddonMgr/ru.md). Любые сторонние зависимости Assembly3 должны разрешаться автоматически Менеджером Дополнений.
...
-   [AppImage](AppImage/ru.md) разрабатываемой версии; она базируется на текущей главной ветке, и включает зависимости, необходимые для работы с Assembly3, такие как решатель SolveSpace.
...
---
⏵ [documentation index](../README.md) > [Addons](Category_Addons.md) > [External Workbenches](Category_External%20Workbenches.md) > Assembly3 Workbench/ru
