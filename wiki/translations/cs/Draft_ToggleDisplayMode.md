# Draft ToggleDisplayMode/cs
---
- GuiCommand:   Name: Draft ToggleDisplayMode   Name/cs: Přepnout zobrazovací mód   Workbenches: Draft_Workbench/cs   Kreslení, Arch_Workbench/cs|MenuLocation: Kreslení -> Utility -> Přepnout zobrazovací mód   Shortcut: Shift + Space---


</div>

## Description


<div class="mw-translate-fuzzy">

## Popis

Tento nástroj přepíná zobrazení vybraného objektu(ů) mezi Drátovým zobrazením a mezi Plochými čarami


</div>

<img alt="" src=images/Draft_ToggleDisplayMode_example.png  style="width:400px;"> 
*On the left the larger cube obstructs the cube behind it. On the right the Display Mode of the larger cube is set to Wireframe making it possible to select a face belonging to the smaller cube.*

## Usage


<div class="mw-translate-fuzzy">

## Použití

1.  Vyberte objekt(y)
2.  Stiskněte tlačítko **<img src="images/Draft_ToggleDisplayMode.png" width=16px> [Přepnout zobrazovací mód](Draft_ToggleDisplayMode/cs.md)
** nebo stiskněte klávesy **SHIFT** a **MEZERNÍK**


</div>



---
⏵ [documentation index](../README.md) > [Draft](Draft_Workbench.md) > Draft ToggleDisplayMode/cs
