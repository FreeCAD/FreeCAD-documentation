...
在文档中，可以将对象移动到组中，并赋予唯一的名称。管理组、对象和对象名主要是从树视图中完成的。当然，就像freecad中的所有操作一样，它也可以通过[Python](python.md)解释器完成。在树视图中，可以创建组、将对象移动到组、删除对象或组、在树视图中或对象上单击鼠标右键、通过双击对象的名称重命名对象或其他的操作（取决于当前工作台）。
...
FreeCad文档中的对象可以是不同类型的。每个工作台都可以创建自己的对象类型，例如[网格工作台](Mesh_Workbench.md)用来创建网格对象，[零件工作台](Part_Workbench.md)用来创建零件对象，[草稿工作台](Draft_Workbench.md)也可以用来创建零件对象等。
...
可以通过[Python](python.md)解释器轻松创建、访问和修改文档。例如：
