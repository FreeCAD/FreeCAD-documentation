...
-   **（ZIPベースの）複合ドキュメント保存フォーマット**: FreeCADドキュメントは.[fcstd](File_Format_FCStd/ja.md)拡張子で保存できます。これにはジオメトリー、スクリプト、サムネイルアイコンといったさまざまな種類の情報を保存することができます。
...
---
⏵ [documentation index](../README.md) > [User Documentation](Category_User%20Documentation.md) > Feature list/ja
