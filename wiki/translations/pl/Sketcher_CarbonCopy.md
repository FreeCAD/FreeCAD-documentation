...
-   Ponieważ skopiowane przez kalkę techniczną wiązania wymiarowe używają wyrażeń, są one renderowane w innym kolorze. Kolor ten można dostosować za pomocą opcji **Edycja → Preferencje → Szkicownik → Kolory dla więzów → Wiązanie zależne od wyrażenia** w [Edytorze ustawień](Preferences_Editor/pl.md).
