...
Polecenie <img alt="" src=images/Draft_Wire.svg  style="width:24px;"> **Polilinia** [tworzy](#Utwórz.md) linię łamaną, czyli sekwencję kilku połączonych segmentów linii. Polecenie to może być również użyte do [łączenia](#Przyłącz.md) [linii](Draft_Line/pl.md) i polilinii.
...
-    **Wzór|Enumeration**: określa rodzaj [wypełnienia](Draft_Pattern/pl.md), którym ma być pokryta powierzchnia zamkniętej linii. Ta właściwość działa tylko wtedy, gdy właściwość {{PropertyData/pl|Utwórz powierzchnię}} ma wartość {{TRUE/pl}} i gdy {{PropertyView/pl|Display Mode}} ma wartość {{value|Flat Lines}}.
