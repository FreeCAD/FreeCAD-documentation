...
2.  Na przykład, jeśli ciąg znaków to {{Incode|"Solver Calculix Standard"}}, znajdziesz ten plik: **src/Mod/Fem/femcommands/commands.py**, i będziesz wiedział, że ten ciąg należy do środowiska pracy MES.
...
4.  Istnieją dwie możliwości:
    ...
    2.  Ciąg znajduje się na Crowdin. Istnieje więc kilka możliwości:
        -   Może być tak, że najnowsze ciągi Crowdin nie zostały jeszcze scalone. Możesz sprawdzić, czy tak jest, wyszukując ten ciąg *(patrz wyżej)* i sprawdzając, czy pojawia się on w przetłumaczonych plikach **.ts**. W naszym przykładzie {{Incode|"Solver Calculix Standard"}} znalazłby się w **src/Mod/Fem/Gui/Resources/translations/Fem_pl.ts**. Oznacza to, że przetłumaczony ciąg znaków znajduje się w źródle programu FreeCAD i wszystko powinno być w porządku w następnej kompilacji.
