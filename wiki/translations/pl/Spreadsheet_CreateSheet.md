---
- GuiCommand:/pl
   Name:Spreadsheet CreateSheet
   Name/pl:Arkusz kalkulacyjny: Utwórz nowy arkusz
   MenuLocation:Arkusz kalkulacyjny → Utwórz arkusz
   Workbenches:[Arkusz kalkulacyjny](Spreadsheet_Workbench/pl.md)
   Version:0.17
---

# Spreadsheet CreateSheet/pl

## Opis

Przycisk **<img src=images/Spreadsheet_CreateSheet.svg style="width:16px"> [Tworzenie arkusza](Spreadsheet_CreateSheet/pl.md)** dodaje do aktywnego dokumentu nowy obiekt arkusza kalkulacyjnego, który zostanie otwarty w nowej zakładce w obszarze [głównego obszaru widoku](main_view_area/pl.md)

## Użycie

1.  Naciśnij przycisk **<img src=images/Spreadsheet_CreateSheet.svg style="width:16px">   [Tworzenie arkusza](Spreadsheet_CreateSheet/pl.md)**.
2.  Edytuj zawartość komórek.
3.  Zamknij zakładkę arkusza kalkulacyjnego, gdy skończysz, wartości będą zapisywane automatycznie w miarę ich dodawania.





{{Spreadsheet_Tools_navi

}}

---
[documentation index](../README.md) > [Spreadsheet](Spreadsheet_Workbench.md) > Spreadsheet CreateSheet/pl
