---
- GuiCommand:/pl
   Name:Sketcher Reorient
   Name/pl:Szkicownik: Zmień orientację szkicu
   MenuLocation:Szkic → Zmień orientację szkicu
   Workbenches:[Szkicownik](Sketcher_Workbench/pl.md), [Projekt Części](PartDesign_Workbench/pl.md)
   SeeAlso:[Mapuj szkic](Sketcher_MapSketch/pl.md), [Nowy szkic](Sketcher_NewSketch/pl.md)
---

## Opis

Pozwala na oderwanie szkicu od powierzchni i przymocowanie go do jednej z głównych płaszczyzn *(z przesunięciem)*.

## Użycie

1.  W widoku drzewa wybierz szkic, który ma zostać zmieniony.
2.  Wybierz z menu głównego {{MenuCommand|Sketch → <img src=images/Sketcher_ReorientSketch.svg style="width:16px"> [Zmień orientację szkicu](Sketcher_ReorientSketch.md)}}.
3.  Pojawi się menu z pytaniem, czy chcesz odłączyć szkic od jego powierzchni.
4.  Wybierz **Tak** uzyskasz identyczne menu co w **Sketch** → **Nowy szkic**.
5.  Możesz podłączyć szkic do jednej z głównych płaszczyzn.
6.  Masz również możliwość dodania przesunięcia w kierunku dodatnim lub ujemnym.

## Uwagi

-   Po wybraniu **Anuluj** w drugim menu szkic jest po prostu oderwany od powierzchni.
-   Dopóki nie zostanie wybrany szkic, komenda jest nieaktywna.





{{Sketcher Tools navi

}}  
