---
 GuiCommand:
   Name: TechDraw ExtensionInsertDiameter
   Name/fr: TechDraw Diamètre
   MenuLocation: TechDraw , Extensions : cotes , Insérer le préfixe "⌀"
   Workbenches: TechDraw_Workbench/fr
   Shortcut: 
   Version: 0.20
   SeeAlso: TechDraw_ExtensionInsertSquare, TechDraw_ExtensionInsertRepetition/fr
---

# TechDraw ExtensionInsertDiameter/fr

## Description

L\'outil **TechDraw Diamètre** insère un symbole \"⌀\" au début du texte de la cote.

<img alt="" src=images/TechDraw_ExtensionInsertDiameterExample.png  style="width:600px;"> 
*A droite, les cotes avec le symbole inséré*



## Utilisation

1.  Sélectionnez une ou plusieurs cotes.
2.  Il existe plusieurs façons de lancer l\'outil :
    -   Appuyez sur le bouton **<img src="images/TechDraw_ExtensionInsertDiameter.svg" width=16px> [Insérer le préfixe "⌀"](TechDraw_ExtensionInsertDiameter/fr.md)**.
    -   Sélectionnez la commande **TechDraw → Extensions : cotes → <img src="images/TechDraw_ExtensionInsertDiameter.svg" width=16px> Insérer un préfixe "⌀"** à partir du menu.
3.  Le symbole \"⌀\" est inséré dans le texte de cote des cotes sélectionnées.





{{TechDraw_Tools_navi

}}



---
⏵ [documentation index](../README.md) > [TechDraw](TechDraw_Workbench.md) > TechDraw ExtensionInsertDiameter/fr
