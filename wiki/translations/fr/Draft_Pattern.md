...
5.  Passez à l\'onglet {{MenuCommand|Visualisation}} de l\'[Éditeur de propriétés](Property_editor/fr.md).
...
## Motifs disponibles {#motifs_disponibles}
...
-   Pour spécifier un répertoire avec des motifs de hachures supplémentaires : {{MenuCommand|Edition → Préférences... → Draft → Paramètres visuels → Autre emplacement des motifs de hachures SVG}}. Sélectionnez un fichier dans le répertoire, puis supprimez le nom du fichier dans la zone de saisie des préférences, ne laissant que le chemin d\'accès. Après avoir modifié cette préférence, vous devez redémarrer FreeCAD.
-   La fenêtre {{MenuCommand|Edition → Préférences... → Draft → Paramètres visuels → Résolution des motifs de hachures}} préférence n\'est pas utilisée.
-   Pour modifier la taille du motif {{PropertyView/fr|Pattern Size}} utilisée pour les nouveaux objets : {{MenuCommand|Edition → Préférences... → Draft → Paramètres visuels → Taille par défaut du motif des hachures}}.
