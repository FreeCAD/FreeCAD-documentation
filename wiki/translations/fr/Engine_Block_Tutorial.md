 {{UnfinishedDocu}} {{TutorialInfo/fr
|Topic=  Atelier Part
|Level=  Débutant
|Time=   1 heure
|Author= Andrewbuck40
...
Pour commencer, ouvrez FreeCAD , aller à {{MenuCommand|Fichier → Nouveau}} pour créer un nouveau document, puis {{MenuCommand|Fichier → Enregistrer}} pour sauvegarder quelque part sur votre ordinateur, j\'ai nommé le projet **\"Moteur\"**. Vous remarquerez que lorsque vous enregistrez le projet, l*\'arborescence* sur le côté gauche de l\'écran affiche le nom du projet sur lequel vous travaillez. Vous pouvez avoir plus d\'un projet ouvert à la fois et chaque projet sera présenté comme la racine d\'un arbre dans l\'arborescence.

## Dégrossissage du bloc {#dégrossissage_du_bloc}

Maintenant, commençons à travailler sur le modèle réel. Nous allons commencer par l\'ajout d\'une boîte pour le contour global du bloc moteur. Pour cela, nous devons ajouter une *pièce* au modèle à faire, allez dans {{MenuCommand|Affichage → Atelier → Part}} pour sélectionner l\'[atelier Part](Part_Workbench/fr.md). Vous remarquerez que lorsque vous sélectionnez l\'atelier, vous obtenez un jeu de boutons différent sur la barre d\'outils en haut. Voyagez dans d\'autres ateliers pour vous familiariser avec le système d\'ateliers et revenez ensuite à l\'atelier Part (pièce).

### Le bloc cylindre {#le_bloc_cylindre}
...
### Le premier cylindre {#le_premier_cylindre}
...
### Principal avantage de la Modélisation Paramétrique {#principal_avantage_de_la_modélisation_paramétrique}
...
### Bloc et chapeaux de paliers {#bloc_et_chapeaux_de_paliers}
...
### Tailler les paliers {#tailler_les_paliers}
...
### Finition du carter {#finition_du_carter}
...
## Modélisation 2D du dessin du joint de culasse {#modélisation_2d_du_dessin_du_joint_de_culasse}
...
### Disposition des goujons de culasse {#disposition_des_goujons_de_culasse}
...
### L'autre côté du bloc {#lautre_côté_du_bloc}
...
-   Nous pourrions sélectionner les trois que nous avons ajoutés, aller à {{MenuCommand|Édition → Dupliquer la sélection}}, puis inverser les coordonnées Y des trois nouveaux cercles.
...
### Enlever les excédents de matière dans le bloc {#enlever_les_excédents_de_matière_dans_le_bloc}
...
### Extrusion du design de la tête 2D dans notre modèle 3D pour finir le design {#extrusion_du_design_de_la_tête_2d_dans_notre_modèle_3d_pour_finir_le_design}
