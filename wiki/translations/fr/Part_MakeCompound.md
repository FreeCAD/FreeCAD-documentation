...
2.  Faites {{MenuCommand|Pièces → Composé → Créer un composé}} dans le menu Pièce ou cliquez sur le bouton <img alt="" src=images/Part_Compound.svg  style="width:24px;">.
...
Pour activer cette vérification, allez sur {{MenuCommand|Outils → Éditer Paramètres... → Préférences → Mod → Part → CheckGeometry → RunBOPCheck}} et réglez le paramètre sur `true`.
