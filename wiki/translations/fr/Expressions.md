...
Comme indiqué ci-dessus, vous pouvez référencer un objet par sa **Name**. Mais vous pouvez également utiliser sa **Label**. Dans le cas d\'une **Label**, vous devez l\'entourer de double symboles `<<` et `>>`, comme ceci `<<Label>>`.
...
[Les fonctions trigonométriques](https://fr.wikipedia.org/wiki/Fonction_trigonom%C3%A9trique) utilisent le degré comme unité par défaut. Pour les radians, ajoutez première valeur dans une expression. Ainsi, par exemple, `cos(45)` est identique à `cos(pi rad / 4)`. Les expressions en degrés peuvent utiliser soit `deg` soit `°`, par exemple `360deg - atan2(3 ; 4)` ou `360&deg; - atan2(3 ; 4)`. Si une expression est sans unité et doit être convertie en degrés ou en radians pour des raisons de compatibilité, multipliez-la par `1deg`, `1°` ou `1rad` selon le cas, par exemple `(360 - X) * 1deg`; `(360 - X) * 1°`; `(0.5 + pi / 2) * 1rad`.
...
|                                  |                                                                                                                                                                                                              |
+++
|                      | Produit vectoriel de deux vecteurs $v_1 \times v_2$.                                                                                                                                                         |
| `vcross(v1; v2)`           |                                                                                                                                                                                                              |
|                                  |                                                                                                                                                                                                              |
+++
|                      | Produit scalaire de deux vecteurs $v_1 \cdot v_2$.                                                                                                                                                           |
| `v1 * v2`                  |                                                                                                                                                                                                              |
|                                  |                                                                                                                                                                                                              |
+++
|                      |                                                                                                                                                                                                              |
...
En guise de solution de contournement, il est possible d\'afficher une plage de cellules de la deuxième feuille de calcul dans la première (ou vice versa) en créant une [liaison entre cellules](Spreadsheet_Workbench/fr#Liaison_entre_cellules.md) avec l\'option **Masquer la dépendance de la liaison**.
