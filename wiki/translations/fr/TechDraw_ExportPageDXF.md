...
-   Ces paramètres affectent la sortie:
    -   
        {{MenuCommand|Outils → Editer paramètres → BaseApp/Preferences/Mod/Import → DxfVersionOut}}
        
        . Il s\'agit d\'une valeur entière. Les entrées valides sont 12 ou 14. La valeur par défaut est 14.

    -   
        {{MenuCommand|Outils → Editer paramètres → BaseApp/Preferences/Mod/Import → DiscretizeEllipses}}
    ...
    -   
        {{MenuCommand|Outils → Editer paramètres → BaseApp/Preferences/Mod/Import → maxsegmentlength}}
