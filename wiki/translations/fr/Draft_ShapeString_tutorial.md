...
:   3.3. Dans **Chamfer edges** [Panneau des tâches](task_panel/fr.md), se rendre à **Selection** puis choisir **Select edges**. En tant que **Fillet type**, choisissez `Constant length` puis définissez **Length** à `5 mm`.
...
:   5.6. Remplacez **String** en `FreeCAD`; remplacez **Height** en `5 mm`; remplacez **Tracking** en `0 mm`.
:   5.7. Assurez-vous que **Font file** pointe vers une police valide, par exemple `/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf`. Appuyez sur les points de suspension **...** pour ouvrir la boîte de dialogue du système d\'exploitation pour trouver une police.
...
11\. Dans la [vue en arboresence](tree_view/fr.md), sélectionnez `Extrude`, dans l\'onglet **Data** de [éditeur de propriétés](property_editor/fr.md), cliquez sur **Placement** afin que le bouton points de suspension **...** apparaisse à droite.

:   11.1. Cochez l\'option **Apply incremental changes**.
:   11.2. Remplacez **Rotation** par `Rotation axis with angle`; **Axis** en `Z` et **Angle** par `90 deg` puis cliquez sur **Apply**. Cela appliquera une rotation autour de l\'axe Z et remettra à zéro le champ **Angle**.
:   11.3. Remplacez **Rotation** par `Rotation axis with angle`; **Axis** en `Y`, and **Angle** par `45 deg`, puis cliquez sur {{Button | Apply}}. Cela appliquera une rotation autour de l\'axe Y et remettra à zéro le champ **Angle**.
...
---
[documentation index](../README.md) > [Draft](Draft_Workbench.md) > Draft ShapeString tutorial/fr
