---
- GuiCommand   */fr
   Name   *Sketcher SelectVerticalAxis
   Name/fr   *Sketcher Sélection axe vertical
   MenuLocation   *Sketch → Outils d'esquisse → Sélectionner l'axe vertical
   Workbenches   *[Sketcher](Sketcher_Workbench/fr.md)
   Shortcut   ***Z** **V**
   Version   *0.15
---

# Sketcher SelectVerticalAxis/fr

## Description

Sélectionne l\'axe vertical d\'une esquisse.

## Utilisation

1.  Entrez dans le mode d\'édition de l\'esquisse.
2.  Choisissez **Sketch → Outils d'esquisse → [<img src=images/Sketcher_SelectVerticalAxis.png style="width   *16px"> Sélectionner l'axe vertical** dans le menu principal.
3.  L\'axe vertical de l\'esquisse sera sélectionnée.





{{Sketcher_Tools_navi

}}



---
![](images/Right_arrow.png) [documentation index](../README.md) > [Sketcher](Sketcher_Workbench.md) > Sketcher SelectVerticalAxis/fr
