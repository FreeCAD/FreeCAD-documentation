---
- GuiCommand   */fr
   Name   *Sketcher SelectOrigin
   Name/fr   *Sketcher Sélectionner l'origine
   MenuLocation   *Sketch → Outils d'esquisse → Selectionner l'origine
   Workbenches   *[Sketcher](Sketcher_Workbench/fr.md)
   Shortcut   ***Z** **O**
   Version   *0.15
---

# Sketcher SelectOrigin/fr

## Description

Sélectionne l\'origine d\'une esquisse.

## Utilisation

1.  Entrez dans le mode d\'édition de l\'esquisse.
2.  Choisissez **Sketch → Outils d'esquisse → [<img src=images/Sketcher_SelectOrigin.svg style="width   *16px"> Sélectionnez l'origine** dans le menu principal.
3.  L\'origine de l\'esquisse sera sélectionnée.





{{Sketcher_Tools_navi

}}



---
![](images/Right_arrow.png) [documentation index](../README.md) > [Sketcher](Sketcher_Workbench.md) > Sketcher SelectOrigin/fr
