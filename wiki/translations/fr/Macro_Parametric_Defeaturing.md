...
|Description=Cette macro est similaire à l'outil [Supprimer la fonction](Part_Defeaturing/fr.md) de [atelier Part](Part_Workbench/fr.md), mais elle est paramétrique et fonctionne également dans [PartDesign](PartDesign_Workbench/fr.md). Pour l'utiliser : sélectionnez les faces des fonctions que vous souhaitez supprimer du modèle et exécutez la macro. Les faces sont utilisées dans le defeaturing, pas les arêtes ou les sommets. Les faces utilisées peuvent être modifiées ultérieurement et l'objet se reconstruira automatiquement.<br/>
