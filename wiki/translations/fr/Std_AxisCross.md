...
-   FreeCAD peut afficher un indicateur de système de coordonnées plus petit dans le coin inférieur droit des vues 3D : **Édition → Préférences ... → Affichage → Vue 3D → Afficher le système de coordonnées dans le coin**. Voir [Éditeur de préférences](Preferences_Editor/fr#Vue_3D.md).
...
-   La valeur par défaut des axes du repère peut être modifiée dans les préférences : **Édition → Préférences... → Affichage → Vue 3D → Afficher la croix de l'axe par défaut**. Voir [Réglage des préférences](Preferences_Editor/fr#Vue_3D.md).
