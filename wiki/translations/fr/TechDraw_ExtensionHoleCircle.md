---
 GuiCommand:
   Name: TechDraw ExtensionHoleCircle
   Name/fr: TechDraw Axes de centrage de trous/vis
   MenuLocation: TechDraw , Extensions : taraudages/filetages , Axes de centrage des trous/vis
   Workbenches: TechDraw_Workbench/fr
   Shortcut: 
   Version: 0.20
   SeeAlso: TechDraw_ExtensionCircleCenterLines/fr
---

# TechDraw ExtensionHoleCircle/fr

## Description

L\'outil **TechDraw Axes de centrage de trous/vis** ajoute des axes de centrage à des cercles disposés sur une circonférence.

<img alt="" src=images/TechDraw_ExtensionHoleCircleExample.png  style="width:400px;"> 
*A droite, un motif circulaire avec des axes de centrage*



## Utilisation

1.  Spécifiez à votre convenance les attributs de ligne avec l\'outil <img alt="" src=images/TechDraw_ExtensionSelectLineAttributes.svg  style="width:16px;"> [TechDraw Choix des attributs](TechDraw_ExtensionSelectLineAttributes/fr.md).
2.  Sélectionnez trois cercles ou plus formant un motif circulaire.
3.  Il existe plusieurs façons de lancer l\'outil :
    -   Appuyez sur le bouton **<img src="images/TechDraw_ExtensionHoleCircle.svg" width=16px> [Axes de centrage des trous/vis](TechDraw_ExtensionHoleCircle/fr.md)**.
    -   Sélectionnez la commande **TechDraw → Extensions : taraudages/filetages → <img src="images/TechDraw_ExtensionHoleCircle.svg" width=16px> Axes de centrage des trous/vis** à partir du menu.
4.  Les axes de centrage sont ajoutées.





{{TechDraw_Tools_navi

}}



---
⏵ [documentation index](../README.md) > [TechDraw](TechDraw_Workbench.md) > TechDraw ExtensionHoleCircle/fr
