...
La [vue en arborescence](Tree_view/fr.md) apparaît dans l\'onglet {{MenuCommand|Modèle}} de la [Vue combinée](Combo_view/fr.md), l\'un des panneaux les plus importants de l\'[interface](Interface/fr.md). Elle montre tous les objets définis par l\'utilisateur qui font partie d\'un document FreeCAD. L\'arborescence est une représentation de la [Structure d\'un document](Document_structure/fr.md) et indique quelles informations sont enregistrées sur le disque.
...
## Travailler avec la vue en arborescence {#travailler_avec_la_vue_en_arborescence}
...
-    {{MenuCommand|Expression actions}}: [Copy selected](Std_Expressions.md), [Copy active document](Std_Expressions.md), [Copy all documents](Std_Expressions.md), coller. Celles-ci permettent de travailler avec divers documents mais sont désactivées si aucun document n\'est présent.

Une fois un nouveau document créé, les éléments suivants deviennent actifs:

-    {{MenuCommand|Expression actions}}: [Copy active document](Std_Expressions.md), [Copy all documents](Std_Expressions.md).

De plus, les actions [Liens](Std_LinkMake/fr.md) sont disponibles.

-    {{MenuCommand|Link actions}}: [Std Créer un lien](Std_LinkMake/fr.md).
...
### Sélection du document {#sélection_du_document}

Si vous sélectionnez le document actif et cliquez avec le bouton droit, en plus de {{MenuCommand|Expression actions}} et {{MenuCommand|Link actions}}, les commandes suivantes apparaissent :

-    {{MenuCommand|Show hidden items}}: si elle est active, l\'arborescence affichera les éléments masqués.

-    {{MenuCommand|Search}}: affiche un champ de saisie pour rechercher des objets à l\'intérieur du document sélectionné.

-    {{MenuCommand|Close document}}: ferme le document sélectionné en appelant la méthode `closeDocument()` de l\'application.

-    {{MenuCommand|Skip recomputes}}: s\'il est actif, les objets du document ne seront pas [recalculés](Std_Refresh/fr.md) automatiquement.

    -   
        {{MenuCommand|Allow partial recomputes}}
        
        : s\'il est actif, le document n\'autorisera le [recalcul](Std_Refresh/fr.md) que pour certains objets.

-    {{MenuCommand|Mark to recompute}}: marque tous les objets du document comme touchés et prêts pour [recalcul](Std_Refresh/fr.md).

-    {{MenuCommand|[Créer un groupe](Std_Group/fr.md)}}: crée un [groupe](Std_Group/fr.md) dans le document sélectionné en utilisant la méthode `addObject()` du document.
...
-    {{MenuCommand|[Basculer la visibilité](Std_ToggleVisibility/fr.md)}}: rend l\'objet visible ou invisible dans la [vue 3D](3D_view/fr.md).

-    {{MenuCommand|[Afficher la sélection](Std_ShowSelection/fr.md)}}: rend les objets sélectionnés visibles.

-    {{MenuCommand|[Masquer la sélection](Std_HideSelection/fr.md)}}: rend les objets sélectionnés invisibles.

-    {{MenuCommand|[Basculer la sélectivité](Std_ToggleSelectability/fr.md)}}: rend l\'objet non sélectionnable dans la [vue 3D](3D_view/fr.md). Utilisez à nouveau cette commande pour annuler son effet. Il définit l\'attribut `Selectable` de l\'objet sur `True` ou `False`. Modifiez la propriété en basculant {{PropertyView/fr|Selectable}} dans l\'[Éditeur de propriétés](Property_editor/fr.md).

-    {{MenuCommand|[Select all instances](Std_TreeSelectAllInstances.md)}}: sélectionne toutes les instances de cet objet dans l\'arborescence.

-    {{MenuCommand|[Apparence](Std_SetAppearance/fr.md)}}: ouvre la boîte de dialogue pour changer la couleur et la taille des lignes et des sommets et la couleur des faces.

-    {{MenuCommand|[Couleur aléatoire](Std_RandomColor/fr.md)}}: attribue une couleur aléatoire à l\'objet. Il définit l\'attribut `ShapeColor` de l\'objet sur un tuple `(r,g,b)` avec des arborescences aléatoires entre 0 et 1. Modifiez la propriété en modifiant {{PropertyView/fr|Shape Color}} dans l\'[Éditeur de propriétés](Property_editor/fr.md).

-    {{MenuCommand|[Couper](Std_Cut.md)}}: désactivé si le clic droit n\'est pas sur l\'objet.

-    {{MenuCommand|[Copier](Std_Copy.md)}}: copie un objet en mémoire.

-    {{MenuCommand|[Coller](Std_Paste.md)}}: pastes the copied object into the document; the copy is added to the end of the tree view.

-    {{MenuCommand|[Effacer](Std_Delete.md)}}: supprime l\'objet du document et de l\'arborescence en appelant la méthode `removeObject()` du document.

-    {{MenuCommand|Utilities}}: **(optionnel)** commandes contextuelles supplémentaires fournies par l\'[Atelier Draft](Draft_Workbench/fr.md).
...
-    {{MenuCommand|Transform}}: lance le widget de transformation pour déplacer ou faire pivoter l\'objet.

-    {{MenuCommand|Set colors}}: définit les couleurs de l\'objet.

-    {{MenuCommand|Flatten this wire}}: **(Draft)** commande spécifique pour une [Draft Ligne](Draft_Line/fr.md)

-    {{MenuCommand|Hide item}}: si actif, l\'objet sélectionné sera défini comme masqué.

-    {{MenuCommand|Mark to recompute}}: marque l\'objet sélectionné comme coché et prêt pour être [recalculer](Std_Refresh/fr.md).

-    {{MenuCommand|Recompute}}: recalcule l\'objet sélectionné.

-    {{MenuCommand|Rename}}: commence à modifier le nom de l\'objet sélectionné. Cela permet de modifier l\'attribut `Label` mais pas l\'attribut `Name` car ce dernier est en lecture seule.

## Icônes de superposition {#icônes_de_superposition}
...
### ![](images/FreeCAD_Tree_view_recompute.png ) Marque blanche sur fond bleu {#freecad_tree_view_recompute.png_marque_blanche_sur_fond_bleu}
...
### ![](images/FreeCAD_Tree_view_tip.png ) Flèche blanche sur fond vert {#freecad_tree_view_tip.png_flèche_blanche_sur_fond_vert}
...
### ![](images/FreeCAD_Tree_view_unattached.png ) Maillon de chaîne violet sur fond blanc {#freecad_tree_view_unattached.png_maillon_de_chaîne_violet_sur_fond_blanc}
...
### ![](images/FreeCAD_Tree_view_notfullyconstrained.png ) X jaune {#freecad_tree_view_notfullyconstrained.png_x_jaune}
...
### ![](images/FreeCAD_Tree_view_error.png ) Point d\'exclamation blanc sur fond rouge {#freecad_tree_view_error.png_point_dexclamation_blanc_sur_fond_rouge}
