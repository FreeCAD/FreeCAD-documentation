...
**
    -   Sélectionnez l\'option **Maillages → Analyser → <img src="images/Mesh_Evaluation.svg" width=16px> Évaluer et réparer un maillage...** dans le menu.
...
-   Le paramètre **Recherche de points libres "non-manifold"** est stocké    * **Outils → Editer les paramètres... → BaseApp → Preferences → Mod → Mesh → Evaluation → CheckNonManifoldPoints**.
...
-   Le paramètre **Considèrer uniquement les faces de surface nulle comme dégénérées** est stocké    * **Outils → Editer les paramètres... → BaseApp → Preferences → Mod → Mesh → Evaluation → StrictlyDegenerated**.
