...
- GuiCommand:/fr
   ...
   MenuLocation:Drafting → Polygone
...
Un Draft Polygone peut passer du mode inscrit au mode circonscrit en modifiant sa propriété {{PropertyData/fr|Draw Mode}}. Les coins d\'un Draft Polygone peuvent recevoir un congé (arrondis) ou chanfreinés en modifiant respectivement les propriétés {{PropertyData/fr|Fillet Radius}} ou {{PropertyData/fr|Chamfer Size}}.
...
1.  Il existe plusieurs façons de lancer la commande :
    ...
    -   Sélectionnez l\'option {{MenuCommand|Drafting → <img src="images/Draft_Polygon.svg" width=16px> Polygone}} dans le menu.
    -   Utilisez le raccourci clavier : **P** puis **G**.
2.  Le panneau de tâches {{MenuCommand|Polygone}} s\'ouvre. Voir [Options](#Options.md) pour plus d\'informations.
3.  Réglez le nombre souhaité de {{MenuCommand|Côtés}}.
...
5.  Choisissez le deuxième point dans la [Vue 3D](3D_view/fr.md) ou entrez un {{MenuCommand|Rayon}}.
...
-   Appuyez sur **L** ou cliquez sur la case {{MenuCommand|Rempli}} pour activer le mode de remplissage. Si le mode rempli est activé, le polygone créé aura la valeur {{PropertyData/fr|Make Face}} `True` et aura une face remplie.
-   Appuyez sur **T** ou cliquez sur la case {{MenuCommand|Continuer}} pour activer le mode continu. Si le mode continu est activé, la commande redémarre après avoir terminé, ce qui vous permet de continuer à créer des polygones.
...
-   Si l\'option {{MenuCommand|Édition → Préférences... → Draft → Paramètres généraux → Options des outils de Draft → Utiliser les primitives de Part si possible}} est cochée, la commande créera un [Part Polygone régulier](Part_RegularPolygon/fr.md) au lieu d\'un Draft Polygone.
...
-    {{PropertyData/fr|Area|Area}}: (en lecture seule) spécifie la surface de la face du polygone. La valeur sera {{value|0.0}} si {{PropertyData/fr|Make Face}} est `False`.

-    {{PropertyData/fr|Chamfer Size|Length}}: spécifie la longueur des chanfreins aux coins du polygone.

-    {{PropertyData/fr|Draw Mode|Enumeration}}: spécifie si le polygone est {{value|inscrit}} dans un cercle ou {{value|circonscrit}} autour d\'un cercle.

-    {{PropertyData/fr|Faces Number|Integer}}: spécifie le nombre de côtés du polygone.

-    {{PropertyData/fr|Fillet Radius|Length}}: spécifie le rayon des congés aux coins du polygone.

-    {{PropertyData/fr|Make Face|Bool}}: spécifie si le polygone forme une face ou non. Si c\'est `True`, une face est créée, sinon seul le périmètre est considéré comme faisant partie de l\'objet.

-    {{PropertyData/fr|Radius|Length}}: spécifie le rayon du cercle qui définit le polygone.
...
-    {{PropertyView/fr|Pattern|Enumeration}}: spécifie un [Draft Motif](Draft_Pattern/fr.md) avec lequel remplir la face du polygone. Cette propriété ne fonctionne que si {{PropertyData/fr|Make Face}} est `True` et si {{PropertyData/fr|Make Face}} est à {{value|Flat Lines}}.
