...
2.  Il existe plusieurs façons d\'invoquer la commande :
    ...
    -   Sélectionnez l\'option {{MenuCommand|Modification → Array tools → <img src="images/Draft_PolarArray.svg" width=16px> Réseau polaire}} dans le menu.
3.  Le panneau de tâches {{MenuCommand|Réseau polaire}} s\'ouvre. Voir [Options](#Options.md) pour plus d\'informations.
...
-   Entrez l\'{{MenuCommand|Angle polaire}} pour spécifier l\'angle total du réseau. L\'angle est positif dans le sens inverse des aiguilles d\'une montre.
-   Entrez le {{MenuCommand|Nombre d'éléments}}. Doit être au moins {{Value|2}}. Le maximum qui peut être saisi dans le panneau de tâches est {{Value|99}}, mais des valeurs plus élevées sont possibles en modifiant la propriété {{PropertyData/fr|Number Polar}} du réseau.
...
-   Appuyez sur le bouton {{MenuCommand|Reset point}} pour réinitialiser le {{MenuCommand|Centre de rotation}} à l\'origine.
...
-   Si la case {{MenuCommand|Lier un réseau}} est cochée, un réseau de liens est créé au lieu d\'un réseau régulier. Un réseau lié (Link) est plus efficace car ses éléments sont des objets [App Link](App_Link/fr.md).
...
-   Pour modifier le nombre de décimales utilisées pour la saisie des coordonnées et des angles : {{MenuCommand|Edition → Préférences... → Général → Unités → Systèmes d'unités → Nombre de décimales}}.
