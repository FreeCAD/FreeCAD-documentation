...
2.  Par exemple, si la chaîne est {{Incode|"Solver Calculix Standard"}} vous trouverez ce fichier : {{FileName|src/Mod/Fem/femcommands/commands.py}}, et vous saurez que la chaîne appartient à l\'atelier FEM.
...
4.  Il y a deux possibilités :
    1.  La chaîne de caractères n\'est pas sur Crowdin parce qu\'elle n\'a pas été récupérée par le script qui rassemble les chaînes de caractères du code source de FreeCAD et les empaquette dans des fichiers {{FileName|.ts}}. Il peut y avoir deux raisons à cela :
