...
Dans la nouvelle page, vous devez utiliser le _ en haut, avec un minimum d\'informations:
...
-   Utilisez la page _ pour télécharger votre icône personnalisée aux formats [SVG](SVG/fr.md) ou PNG. Elle devrait avoir le même nom que la macro.
...
_ mettra les informations sur l\'utilisation et l\'installation des macros dans chaque page.
...
Dans la section **Script**, utilisez _ pour placer le code de la macro dans la page. Cela créera un bloc de texte utilisant la police de caractères monospace, qui préservera les espaces essentiels pour [Python](Python/fr.md).
...
Ce _ génère essentiellement un bloc de balises HTML <pre> ... </pre> de sorte que celles-ci peuvent être utilisées directement au lieu d\'utiliser le modèle. Le [Gestionnaire d\'Addon](Std_AddonMgr/fr.md) recherchera le plus gros bloc de ce type et l\'utilisera pour le corps de la macro.
...
Si votre macro est trop grosse et dépasse 64 Ko, elle ne pourra pas être hébergée sur le wiki. Dans ce cas, utilisez le _ avec un lien vers l\'adresse Web de la page du code brut (page texte du code).
...
Utilisez le template _ pour inclure une ligne dans la catégorie appropriée de la [Liste des Macros](Macros_recipes/fr.md). Créez une nouvelle catégorie si nécessaire.
...
Vous pouvez également utiliser le paramètre facultatif Icon= pour spécifier le fichier image qui sera placé au début de la ligne. L\'icône doit être un fichier _. 
```python
<nowiki>
