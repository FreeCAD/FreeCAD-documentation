...
Le **Balayage additif** crée un solide dans le corps actif en balayant une ou plusieurs esquisses (également appelées sections transversales) le long d\'un chemin ouvert ou fermé. Si le corps contient déjà des éléments, le balayage additif sera fusionné à eux.
...
2.  **Arranger** correctement les deux formes en 3D. Il est recommandé de placer l\'origine de la section transversale sur la ligne de la trajectoire. Les deux esquisses doivent dans la plupart des cas être **orthogonales**. Cela peut être fait avec la fonction \"Map Mode\" (rendre les deux esquisses visibles avec **Espace**). Sélectionner l\'esquisse de la section transversale. Sélectionner Properties/DataTab/MapMode. Cliquer sur le bouton **...** qui apparaît à droite. Dans la boîte de dialogue, sélectionner un sommet de l\'esquisse de trajectoire et sélectionner le mode approprié pour aligner correctement les deux esquisses.
3.  Il existe plusieurs façons de lancer l\'outil :
    -   appuyer sur le bouton **<img src="images/PartDesign_AdditivePipe.svg" width=16px> [Balayage additif](PartDesign_AdditivePipe/fr.md)**.
    -   sélectionner l\'option **PartDesign → Créer une fonction additive → <img src="images/PartDesign_AdditivePipe.svg" width=16px> Balayage additif** du menu.
4.  Dans **Paramètres de balayage**, sous **Trajectoire à balayer tout le long**, appuyer sur le bouton **Objet**.
5.  Sélectionner l\'esquisse à utiliser comme trajectoire dans la vue 3D. Dans ce cas, l\'esquisse entière sera utilisée comme trajectoire.
...
-    **Spine|LinkSub**: chemin à suivre.

-    **Spine Tangent|Bool**: true ou false (par défaut). True étend le chemin pour inclure les arêtes tangentes.

-    **Auxiliary Spine|LinkSub|Hidden**: chemin secondaire permettant d\'orienter le balayage.

-    **Auxiliary Spine Tangent|Bool**: true ou false (par défaut). True étend chemin auxiliaire pour inclure les arêtes tangentes.

-    **Auxiliary Curvelinear|Bool**: true ou false (par défaut). True calcule la normale entre des points équidistants sur les deux chemins.
