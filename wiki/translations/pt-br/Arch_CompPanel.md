---
- GuiCommand:
   Name:Arch CompPanel
   MenuLocation:Arch → Panel
   Workbenches:[Arch](Arch_Workbench.md), [BIM](BIM_Workbench.md)
   Version:0.17
   SeeAlso:
---

# Arch CompPanel/pt-br

## Descrição

Grouping of several Panel related tools.

## Tipos

-   <img alt="" src=images/Arch_Panel.svg  style="width:32px;"> [Panel](Arch_Panel.md): Creates a panel object from a selected 2D object <small>(v0.15)</small> 
-   <img alt="" src=images/Arch_Panel_Cut.svg  style="width:32px;"> _ <small>(v0.17)</small> 
-   <img alt="" src=images/Arch_Panel_Sheet.svg  style="width:32px;"> _ or other 2D objects <small>(v0.17)</small> 
-   <img alt="" src=images/Arch_Nest.svg  style="width:32px;"> [Nest](Arch_Nest.md): Allow to nest several flat objects inside a container shape <small>(v0.17)</small> 


<div class="mw-translate-fuzzy">





</div>

---
[documentation index](../README.md) > [Arch](Category_Arch.md) > [Arch](Arch_Workbench.md) > Arch CompPanel/pt-br
