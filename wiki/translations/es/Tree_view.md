...
## Trabajando con el vista arbol {#trabajando_con_el_vista_arbol}
...
-    {{MenuCommand|Expression actions}}: [Copy selected](Std_Expressions.md), [Copy active document](Std_Expressions.md), [Copy all documents](Std_Expressions.md), Paste. These allow working with various documents, but are disabled if no document is present.
...
### Seleccionar el documento {#seleccionar_el_documento}
...
-    {{MenuCommand|Close document}}: closes the selected document by calling the application\'s `closeDocument()` method.

-    {{MenuCommand|Skip recomputes}}: if active, the document\'s objects will not [recompute](Std_Refresh.md) automatically.
...
-    {{MenuCommand|Mark to recompute}}: marks all objects of the document as touched, and ready for [recompute](Std_Refresh.md).

-    {{MenuCommand|[Create group](Std_Group.md)}}: creates a [group](Std_Group.md) in the selected document by using the document\'s `addObject()` method.

### Seleccionar de objetos {#seleccionar_de_objetos}
...
-    {{MenuCommand|[Toggle visibility](Std_ToggleVisibility.md)}}: makes the object visible or invisible in the [3D view](3D_view.md).

-    {{MenuCommand|[Show selection](Std_ShowSelection.md)}}: makes the selected objects visible.

-    {{MenuCommand|[Hide selection](Std_HideSelection.md)}}: makes the selected objects invisible.

-    {{MenuCommand|[Toggle selectability](Std_ToggleSelectability.md)}}: makes the object no longer selectable in the [3D view](3D_view.md); use again this command to cancel its effect. It sets the object\'s `Selectable` attribute to `True` or `False`. Change the property by toggling **Selectable** in the [property editor](Property_editor.md).

-    {{MenuCommand|[Select all instances](Std_TreeSelectAllInstances.md)}}: selects all instances of this object in the tree view.
...
-    {{MenuCommand|[Cut](Std_Cut.md)}}: disabled if the right-click is not on the object.

-    {{MenuCommand|[Copy](Std_Copy.md)}}: copies an object into memory.

-    {{MenuCommand|[Paste](Std_Paste.md)}}: pastes the copied object into the document; the copy is added to the end of the tree view.

-    {{MenuCommand|[Delete](Std_Delete.md)}}: removes the object from the document, and from the tree view, by calling the document\'s `removeObject()` method.

-    {{MenuCommand|Utilities}}: **(optional)** additional contextual commands provided by the [Draft Workbench](Draft_Workbench.md).
...
-    {{MenuCommand|Flatten this wire}}: **(Draft)** specific command for a [Draft Line](Draft_Line.md).

-    {{MenuCommand|Hide item}}: if active, the selected object will be set as hidden.

-    {{MenuCommand|Mark to recompute}}: marks the selected object as touched, and ready for [recompute](Std_Refresh.md).

-    {{MenuCommand|Recompute}}: recomputes the selected object.

-    {{MenuCommand|Rename}}: starts editing the name of the selected object. This allows changing the `Label` attribute, but not the `Name` attribute, as the latter is read-only.

## Iconos superpuestos {#iconos_superpuestos}
...
### ![](images/FreeCAD_Tree_view_recompute.png ) Marca de verificación blanca sobre fondo azul {#freecad_tree_view_recompute.png_marca_de_verificación_blanca_sobre_fondo_azul}
...
### ![](images/FreeCAD_Tree_view_tip.png ) Flecha blanca sobre fondo verde {#freecad_tree_view_tip.png_flecha_blanca_sobre_fondo_verde}
...
### ![](images/FreeCAD_Tree_view_unattached.png ) Cadena de eslabones púrpura sobre fondo blanco {#freecad_tree_view_unattached.png_cadena_de_eslabones_púrpura_sobre_fondo_blanco}
...
### ![](images/FreeCAD_Tree_view_notfullyconstrained.png ) Amarillo X {#freecad_tree_view_notfullyconstrained.png_amarillo_x}
...
### ![](images/FreeCAD_Tree_view_error.png ) Signo de exclamación blanco sobre fondo rojo {#freecad_tree_view_error.png_signo_de_exclamación_blanco_sobre_fondo_rojo}
