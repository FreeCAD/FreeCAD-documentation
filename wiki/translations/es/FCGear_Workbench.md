# <img alt="Icono de entorno de trabajo externo FCGear" src=images/FCGear_workbench_icon.svg  style="width:64px;"> FCGear Workbench/es






## Introducción

El **entorno de trabajo FCGear**es un [entorno de trabajo externo](External_workbenches/es.md) para fabricar diferentes tipos de engranajes y ruedas helicoidales en FreeCAD. El modelado paramétrico permite cambiar las geometrías requeridas en cualquier momento. Por ejemplo, cambiando unos pocos parámetros, el engranaje evolvente se convierte en un engranaje recto, en un engranaje helicoidal o en un engranaje doblemente helicoidal.

Para que los resultados de FCGear sean utilizables, es necesario tener ciertos conocimientos básicos sobre los diferentes tipos de engranajes. Módulo, diámetro de paso o diámetro de raíz son términos comunes y, por tanto, deben conocerse.

Junto con la impresión 3D, los usuarios domésticos tienen ahora la oportunidad de diseñar y producir engranajes y ruedas dentadas sinfín según sus propias ideas personales y, si es necesario, adaptarlas a las condiciones constructivas.

El entorno de trabajo FCGear se puede instalar a través del <img alt="" src=images/AddonManager.svg  style="width:24px;"> [gestor de complementos](Std_AddonMgr/es.md). Para la instalación manual, consulte [Instalación de más entornos de trabajo](Installing_more_workbenches.md).

Después de la instalación, las herramientas están disponibles en la barra de herramientas de Gear y en el menú Gear.



## Herramientas

-   <img alt="" src=images/FCGear_InvoluteGear.svg  style="width:32px;"> [Engranaje evolvente](FCGear_InvoluteGear/es.md): Crea un engranaje evolvente externo.


<div class="mw-translate-fuzzy">

-   <img alt="" src=images/FCGear_InternalInvoluteGear.svg  style="width:32px;"> [Engranaje evolvente interno](FCGear_InternalInvoluteGear/es.md): Crea un engranaje evolvente interno.


</div>

-   <img alt="" src=images/FCGear_InvoluteRack.svg  style="width:32px;"> [Cremallera evolvente](FCGear_InvoluteRack/es.md): Crea una cremallera evolvente.

-   <img alt="" src=images/FCGear_CycloidGear.svg  style="width:32px;"> [Engranaje ciclode](FCGear_CycloidGear/es.md) Crea un engranaje cicloide.

-   <img alt="" src=images/FCGear_CycloidRack.svg  style="width:32px;"> [Cremallera cicloide](FCGear_CycloidRack/es.md): Crea una cremallera cicloide.

-   <img alt="" src=images/FCGear_BevelGear.svg  style="width:32px;"> [Engranaje cónico](FCGear_BevelGear/es.md): Crea un engranaje cónico.

-   <img alt="" src=images/FCGear_CrownGear.svg  style="width:32px;"> [Engranaje de corona](FCGear_CrownGear/es.md): Crea un engranaje de corona.

-   <img alt="" src=images/FCGear_WormGear.svg  style="width:32px;"> [Tornillo sin fin](FCGear_WormGear/es.md): Crea un tornillo sin fin.

-   <img alt="" src=images/FCGear_TimingGearT.svg  style="width:32px;"> [Timing GearT](FCGear_TimingGearT.md): Creates a timing gear T-shape.

-   <img alt="" src=images/FCGear_TimingGear.svg  style="width:32px;"> [Piñon de sincronización](FCGear_TimingGear/es.md): Crear un piñon de sincronización.

-   <img alt="" src=images/FCGear_LanternGear.svg  style="width:32px;"> [Piñon de linterna](FCGear_LanternGear/es.md): Crear un piñon de linterna.

-   <img alt="" src=images/FCGear_HypoCycloidGear.svg  style="width:32px;"> [Engranaje hipocicloide](FCGear_HypoCycloidGear/es.md): Crea un engranaje hipocicloide con sus pasadores.

-   <img alt="" src=images/FCGear_GearConnector.svg  style="width:32px;"> [Combinar dos engranajes](FCGear_GearConnector/es.md): Combina dos engranaje.



## Tipos de engranajes 



### Engranaje evolvente 

<img alt="" src=images/Involute-Gear_example.png  style="width:200px;"> 
*From left to right: Spur gearing, helical gearing, double helical gearing (see [FCGear InvoluteGear](FCGear_InvoluteGear.md))*

### Internal involute gear 

<img alt="" src=images/FCGear_InternalInvoluteGear-01.png  style="width:200px;"> 
*From left to right: Spur gearing, helical gearing, double helical gearing (see [FCGear InternalInvoluteGear](FCGear_InternalInvoluteGear.md))*

### Involute rack 

<img alt="" src=images/Involute-Rack_example.png  style="width:" height="200px;"> 
*From left to right: Spur gearing, helical gearing, double helical gearing (See [FCGear InvoluteRack](FCGear_InvoluteRack.md))*



### Engranaje cicloide 

<img alt="" src=images/Cycloid-Gear_example_1.png  style="width:200px;"> 
*From left to right: Spur gearing, helical gearing, double helical gearing (see [FCGear CycloidGear](FCGear_CycloidGear.md))*

### Cycloid rack 

<img alt="" src=images/FCGear_CycloidRack-01.png  style="width:" height="200px;"> 
*From left to right: Spur gearing, helical gearing, double helical gearing (See [FCGear CycloidRack](FCGear_CycloidRack.md))*



### Engranaje cónico 

<img alt="" src=images/Bevel-Gear_example.png  style="width:150px;"> 
*From left to right: Spur gearing, spiral gearing (see [FCGear BevelGear](FCGear_BevelGear.md))*



### Engranaje de corona 

<img alt="" src=images/Crown-Gear_example.png  style="width:150px;"> 
*Crown gear (see [FCGear CrownGear](FCGear_CrownGear.md))*



### Tornillo sin fin 

<img alt="" src=images/Worm-Gear_example.png  style="width:150px;"> 
*Worm gear (see [FCGear WormGear](FCGear_WormGear.md))*

### Timing gear 

<img alt="" src=images/Timing-Gear_example.png  style="width:150px;"> 
*Timing gearing (see [FCGear TimingGear](FCGear_TimingGear.md))*

### Lantern gear 

<img alt="" src=images/Lantern-Gear_example.png  style="width:150px;"> 
*Lantern gearing (see [FCGear LanternGear](FCGear_LanternGear.md))*

### Hypocycloid gear 

<img alt="" src=images/FCGear_FCGear_HypoCycloidGear-05.png  style="width:120px;"> 
*Hypocycloid gearing (see [FCGear HypoCycloidGear](FCGear_HypoCycloidGear.md))*



## Referencias

-   Author: looooo
-   Home page: <https://github.com/looooo/FCGear>
-   Source code on github: <https://github.com/looooo/FCGear>

## Links to FCGear Workbench 

-   Wiki de FreeCAD: [Macro_FCGear](Macro_FCGear/es.md)

* Foro de FreeCAD: [Bevel gear](http://forum.freecadweb.org/viewtopic.php?f=3&t=12878) y [FCGear](http://forum.freecadweb.org/viewtopic.php?f=21&t=12968)
*Tutoriales: 
* Videos: 
* Archivos: 
* Informar errores: informe errores en [https://github.com/looooo/FCGear/issues](https://github.com/looooo/FCGear/issues)



## Otros enlaces útiles 

-   <img alt="PartDesign_InvoluteGear" src=images/PartDesign_InvoluteGear.svg  style="width:24px;"> [PartDesign InvoluteGear](PartDesign_InvoluteGear/es.md): Esta herramienta permite la creación de un perfil 2D de un engranaje evolvente. Este perfil 2D es completamente paramétrico y se puede rellenar con la función [PartDesign Pad](PartDesign_Pad/es.md).

* [External Workbenches](External_workbenches/es.md): Una lista de todos los bancos de trabajo externos actuales de FreeCAD
* [Recetas de macros](Macros_recipes/es.md)
* [Engranaje cicloide (alemán)](https://vivat-geo.de/zykloidenverzahnung.html)
* [Engranaje evolvente (alemán)](https://vivat-geo.de/evolventenverzahnung.html)



---
⏵ [documentation index](../README.md) > [External Workbenches](Category_External Workbenches.md) > [Addons](Category_Addons.md) > [FCGear](Category_FCGear.md) > FCGear Workbench/es
