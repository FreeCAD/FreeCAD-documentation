...
La idea del mismo es introducir al lector al flujo de trabajo básico del <img alt="" src=images/Workbench_Draft.svg  style="width:24px;">[Banco de Trabajo Draft](Draft_Workbench.md).
...
Este tutorial utiliza la notación {{Value|(x, y, z)}} para denotar las coordenadas requeridas para definir puntos en un objeto. La unidad por defecto es el milímetro {{Value|mm}}.
...
1\. Abre FreeCAD y crea un nuevo documento vacío con **Fichero→[<img src=images/Std_New.svg style="width:16px"> [Nuevo](Std_New.md)**.

:   1.1. Cambia al [Banco de Trabajo Draft](Draft_Workbench.md) en el [selector de Banco de Trabajo](Std_Workbench.md) o con el menú **Vista → Banco de Trabajo → [<img src=images/Workbench_Draft.svg style="width:16px"> Draft**.
:   1.2. Asegúrate de entender cómo utilizar el [Editor de propiedades](property_editor.md), en particular las pestañas **Datos** y **Vista** para cambiar las propiedades. Cuando cambies propiedades es posible que tengas que **<img src="images/Std_Refresh.svg" width=16px> [Refrescar](Std_Refresh.md)} para ver el resultado en la [Vista 3D](3D_view.md).
    :1.3. Dado que los objetos Draft son formas planas, se ven mejor desde arriba. Utiliza {{Button|[<img src=images/Draft_SelectPlane.svg style="width:16px"> [Vista arriba](Std_ViewTop.md)** para fijar la [Vista 3D](3D_view.md).
    ...
    :5.2. In the [Snap toolbar](Draft_Snap.md) make sure **[<img src=images/Draft_Snap_Lock.svg style="width:16px"> [Toggle snap](Draft_Snap_Lock.md)** is active, and only **[<img src=images/Draft_Snap_Endpoint.svg style="width:16px"> [Endpoint](Draft_Snap_Endpoint.md)** as well. As you move the pointer onto the arc and close to one of its endpoints, the [<img src=images/Draft_Snap_Endpoint.svg style="width:24px"> [Endpoint](Draft_Snap_Endpoint.md) icon should appear. Also, the target point is emphasized with a large white dot. Click to select this point.
    ...
    [x<img src=images/01_Dr01_Draft_Arc_profile.png style="width:400px">
...
*Closed profile created by two arcs and two lines.*
    ...
    [x<img src=images/02_Dr01_Draft_Rectangle_circle_polygon.png style="width:400px">
...
*Polar array of the small profile centered around the origin.*
    ...
    :11.3. In the [Snap toolbar](Draft_Snap.md) make sure **[<img src=images/Draft_Snap_Lock.svg style="width:16px"> [Toggle snap](Draft_Snap_Lock.md)** is active, and only **[<img src=images/Draft_Snap_Midpoint.svg style="width:16px"> [Midpoint](Draft_Snap_Midpoint.md)** as well. As you move the pointer to the top edge of the polygon, the [<img src=images/Draft_Snap_Midpoint.svg style="width:24px"> [Midpoint](Draft_Snap_Midpoint.md) icon should appear; click to select this point.
    ...
    12. Repeat the process for the two arcs of the closed profile. The first point of the measurement will still be the origin, and the second point will use the [<img src=images/Draft_Snap_Midpoint.svg style="width:24px"> [Midpoint](Draft_Snap_Midpoint.md) of the arc.

    13. Repeat the process for the circle located in the center. The first point of the measurement will still be the origin. To select the second point make sure **[<img src=images/Draft_Snap_Lock.svg style="width:16px"> [Toggle snap](Draft_Snap_Lock.md)** is active, and only **[<img src=images/Draft_Snap_Angle.svg style="width:16px"> [Angle](Draft_Snap_Angle.md)** as well. As you move the pointer to the top of the circle, the [<img src=images/Draft_Snap_Angle.svg style="width:24px"> [Angle](Draft_Snap_Angle.md) icon should appear; click to select this point. Then move the cursor to the right, and click to fix the dimension.
...
*Dimensions that measure the vertical distance from the origin to the top of the circle, arcs, and polygon.*
    ...
    :14.2. Select the reference point in the [3D view](3D_view.md). In the [Snap toolbar](Draft_Snap.md) make sure **[<img src=images/Draft_Snap_Lock.svg style="width:16px"> [Toggle snap](Draft_Snap_Lock.md)** is active, and only **[<img src=images/Draft_Snap_Midpoint.svg style="width:16px"> [Midpoint](Draft_Snap_Midpoint.md)** as well. Move the pointer to the top edge of the highest arc, so that the [<img src=images/Draft_Snap_Midpoint.svg style="width:24px"> [Midpoint](Draft_Snap_Midpoint.md) icon appears; click to select this point.
...
*Text and ShapeString objects added.*
    ...
    16. Switch to the [<img src=images/Workbench_TechDraw.svg style="width:24px"> [TechDraw Workbench](TechDraw_Workbench.md).
...
*TechDraw page with a projection of the shapes created with the Draft Workbench.*
    ...
    * The [Draft Workbench](Draft_Workbench.md) is intended for 2D drawings which are suitable to be drawn on a grid. The [BIM Workbench](BIM_Workbench.md) mostly builds on top of the tools defined in the [Draft Workbench](Draft_Workbench.md).
