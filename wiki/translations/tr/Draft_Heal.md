---
- GuiCommand:/tr
   Name/tr:İyileştirme
   MenuLocation:Taslak → Araçlar → İyileştirme
   Workbenches:[Taslak](Draft_Workbench/tr.md), [Yapı](Arch_Workbench/tr.md)
   SeeAlso:[Yükselt](Draft_Upgrade/tr.md), [Düşür](Draft_Downgrade/tr.md)
---

# Draft Heal/tr


</div>

## Description


<div class="mw-translate-fuzzy">

## Açıklama

Bu komut, daha eski bir sürümde çalışması için eski bir FreeCAD sürümüyle oluşturulan [Taslak tezgahı](Draft_Workbench/tr.md) nesnelerini düzeltmeye çalışır.


</div>

## Usage


<div class="mw-translate-fuzzy">

## Nasıl kullanılır 

1.  Bir veya daha fazla sorunlu nesne seçin.

2.  
    {{MenuCommand | Taslak → Araçlar → <img src="images/_Draft_Heal.svg_" width= 16px> [ İyileştir](_Draft_Heal/tr.md)}}menüsüne gidin.


</div>


<div class="mw-translate-fuzzy">


{{Draft Tools navi/tr}}


{{Userdocnavi/tr}}


</div>

---
[documentation index](../README.md) > [Draft](Draft_Workbench.md) > Draft Heal/tr
