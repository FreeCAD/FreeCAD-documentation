# Robot CreateTrajectory/tr
---
- GuiCommand:/tr
   Name:Robot CreateTrajectory
   Name/tr:Yörünge oluştur
   Workbenches:[Robot](Robot_Workbench/tr.md)
   MenuLocation:Robot → Yörünge oluştur---


</div>

## Description


<div class="mw-translate-fuzzy">

#### Açıklama

Sahneye yeni bir boş yörünge nesnesi ekler.


</div>

## Usage


<div class="mw-translate-fuzzy">

#### Kullanım

![ 32px](images/_Robot_CreateTrajectory.png ) üzerine tıklayın veya üst menüden {{KEY | Robot}} → {{KEY | <img src="images/_Robot_CreateTrajectory.png_" width= 32px>  Yörünge oluştur}}\'u seçin


</div>


<div class="mw-translate-fuzzy">

### Notlar


</div>





{{Robot_Tools_navi

}}



---
![](images/Button_right.svg) [documentation index](../README.md) > [Robot](Robot_Workbench.md) > Robot CreateTrajectory/tr
