...
Uređivač osobina je jedan od važnijih alata u FreeCAD-u i glavni element rada u FreeCAD-u. Uređivač osobina omogućuje podešavanje osobina objekata u vašem dokumentu.


</div>

The [property editor](property_editor.md) appears when the {{MenuCommand|Model}} tab of the [combo view](combo_view.md) is active in the [interface](interface.md); it allows managing the publicly exposed properties of the objects in the document.
...
Not all properties can be modified always; depending on the specific status of the property, some of them will be invisible (not listed), or be read-only (not editable). {{TOCright}}
...
## Example of Part object properties {#example_of_part_object_properties}
...
## View and Data properties {#view_and_data_properties}
...
## Example of the properties of a PartDesign object {#example_of_the_properties_of_a_partdesign_object}
...
{{docnav/hr|[Postavke Sučelja](Interface_Customization/hr.md)|[Radni Stolovi](Workbenches/hr.md)}}
