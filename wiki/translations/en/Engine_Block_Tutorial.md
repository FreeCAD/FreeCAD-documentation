...
To begin with, open FreeCAD, go to {{MenuCommand|File → New}} to create a new document, and then {{MenuCommand|File → Save}} to save it somewhere on your computer, I named my project **\"Engine\"**. You will notice that after you save the project, the *tree view* on the left side of the screen will show the name of the project you are working on. You can have more than one project open at a time and each project will be shown as the root of a tree in the tree view.

## Roughing Out the Block {#roughing_out_the_block}

Now to begin working on the actual model. We will start by adding a Box for the overall outline of the engine block. To do this we need to add a *part* to the model, go to {{MenuCommand|View → Workbench → Part}} to select the [Part Workbench](Part_Workbench.md). You will notice that after you select the workbench, you get a different set of toolbar buttons at the top. Flip through a couple of the other workbenches to familiarize yourself with the workbench system and then return to the Part Workbench.

### The Billet {#the_billet}
...
### The First Cylinder {#the_first_cylinder}
...
### The Key Advantage of Parametric Modeling {#the_key_advantage_of_parametric_modeling}
