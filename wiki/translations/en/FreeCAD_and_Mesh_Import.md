...
2.  Select the mesh, and go to the {{MenuCommand|Part → [Create shape from mesh](Part_ShapeFromMesh.md)}} or press the <img alt="" src=images/Part_ShapeFromMesh.svg  style="width:24px;"> [Part ShapeFromMesh](Part_ShapeFromMesh.md) button
3.  Click **OK** in the dialog
4.  Select the newly created shape
5.  Go to {{MenuCommand|Part → [Convert to solid](Part_MakeSolid.md)}}
6.  Select the newly created solid
7.  Go to {{MenuCommand|Part → [Refine shape](Part_RefineShape.md)}} or press the <img alt="" src=images/Part_RefineShape.svg  style="width:24px;"> [Part RefineShape](Part_RefineShape.md) button
...
## Third Party Programs {#third_party_programs}
