---
 GuiCommand:
   Name: TechDraw SurfaceFinishSymbols
   MenuLocation: TechDraw , Symbols , Create a Surface Finish Symbol
   Workbenches: TechDraw_Workbench
   Version: 0.21
---

# TechDraw SurfaceFinishSymbols/en

## Description

The **TechDraw SurfaceFinishSymbols** tool adds a surface finish symbol to a page.

<img alt="" src=images/TechDraw_SurfaceFinishSymbolExample1.png  style="width:330px;"> 
*The task panel with the ISO style selected. On the right the created symbol.*

<img alt="" src=images/TechDraw_SurfaceFinishSymbolExample2.png  style="width:330px;"> 
*The task panel with the ASME style selected. On the right the created symbol.*

## Usage

1.  Select an existing view.
2.  There are several ways to invoke the tool:
    -   Press the **<img src="images/TechDraw_SurfaceFinishSymbols.svg" width=16px> [Create a Surface Finish Symbol](TechDraw_SurfaceFinishSymbols.md)** button.
    -   Select the **TechDraw → Symbols → <img src="images/TechDraw_SurfaceFinishSymbols.svg" width=16px> Create a Surface Finish Symbol** option from the menu.
3.  A task panel opens.
4.  Specify the required symbol by clicking one of the 6 symbol buttons.
5.  Optionally change the **Symbol angle**.
6.  Select the **ISO** or the **ASME** style.
7.  Select and/or enter the required attributes.
8.  Press the **OK** button.





{{TechDraw Tools navi

}}



---
⏵ [documentation index](../README.md) > [TechDraw](TechDraw_Workbench.md) > TechDraw SurfaceFinishSymbols/en
