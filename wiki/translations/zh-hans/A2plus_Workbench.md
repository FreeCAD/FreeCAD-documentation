...
一个部件可以包含其他部件。 通过按下工具栏按钮<img alt="" src=images/A2p_ImportPart.svg  style="width:24px;">，然后选择包含部件{{FileName|* .FCStd}}文件，可以像零件一样添加它们。 也可以使用工具栏按钮<img alt="" src=images/A2p_EditPart.svg  style="width:24px;">像零件一样编辑这些子部件。请确保当发生更改后通过工具栏按钮<img alt="" src=images/A2p_ImportPart_Update.svg  style="width:24px;">更新部件关系。
