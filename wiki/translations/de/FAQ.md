...
### Was ist der einfachste Weg, FreeCAD auf meinem Rechner zu installieren ? {#was_ist_der_einfachste_weg_freecad_auf_meinem_rechner_zu_installieren}
...
### Was sind die Voraussetzungen um FreeCAD auszuführen {#was_sind_die_voraussetzungen_um_freecad_auszuführen}
...
#### Für Mac Nutzer {#für_mac_nutzer}

Es wird nur die MacIntel Architektur unterstützt. Für die PowerPC Architektur sind keine Builds verfügbar.

### Was, wenn ich FreeCAD selbst kompilieren möchte? {#was_wenn_ich_freecad_selbst_kompilieren_möchte}
...
### FreeCAD meldet, dass einige Module oder Anwendungen fehlen {#freecad_meldet_dass_einige_module_oder_anwendungen_fehlen}
...
### FreeCAD startet überhaupt nicht {#freecad_startet_überhaupt_nicht}
...
### FreeCAD startet normal, aber nicht alle Symbole werden angezeigt. Einige werden durch ein schwarzes \'X\' ersetzt. {#freecad_startet_normal_aber_nicht_alle_symbole_werden_angezeigt._einige_werden_durch_ein_schwarzes_x_ersetzt.}
...
### Ich habe Anzeigeprobleme, die 3D Ansicht verhält sich nicht korrekt, es gibt Müll, wenn ich die Ansicht bewege/drehe, usw. {#ich_habe_anzeigeprobleme_die_3d_ansicht_verhält_sich_nicht_korrekt_es_gibt_müll_wenn_ich_die_ansicht_bewegedrehe_usw.}
...
### FreeCAD stürzt beim Start ab {#freecad_stürzt_beim_start_ab}
...
#### Die OpenGL Treiber sind nicht installiert oder funktionieren nicht ordnungsgemäß {#die_opengl_treiber_sind_nicht_installiert_oder_funktionieren_nicht_ordnungsgemäß}
...
Und als allgemeiner Tip, um mehr Informationen über Abstürze mit FreeCAD zu erhalten, kannst du es mit dem Programmparameter {{SystemInput|--write-log}} starten. Dadurch wird die Datei {{FileName|FreeCAD.log}} in {{FileName|$HOME/.FreeCAD}} auf Linux, in {{FileName|$HOME/Library/Preferences/FreeCAD}} unter macOS oder {{FileName|%APPDATA%/FreeCAD}} auf Windows Systemen erstellt.
...
#### Eine Bibliothek, die von FreeCAD benötigt wird, ist nicht auf Ihrem System vorhanden oder wurde von FreeCAD nicht gefunden {#eine_bibliothek_die_von_freecad_benötigt_wird_ist_nicht_auf_ihrem_system_vorhanden_oder_wurde_von_freecad_nicht_gefunden}
...
### FreeCAD friert nach dem Hochfahren ein {#freecad_friert_nach_dem_hochfahren_ein}
...
### FreeCAD stürzt beim Erstellen eines neuen Dokuments oder Öffnen einer Datei ab {#freecad_stürzt_beim_erstellen_eines_neuen_dokuments_oder_öffnen_einer_datei_ab}
...
### FreeCAD stürzt nach Bearbeiten → Ausrichtung ab {#freecad_stürzt_nach_bearbeiten_ausrichtung_ab}
...
### Ich habe Schwierigkeiten bei der Ausführung von FreeCAD auf macOS {#ich_habe_schwierigkeiten_bei_der_ausführung_von_freecad_auf_macos}
...
### Ich kann keine numerischen Werte in den Eigenschaftenfeldern von FreeCAD ändern {#ich_kann_keine_numerischen_werte_in_den_eigenschaftenfeldern_von_freecad_ändern}
...
### FreeCAD lief normal, aber plötzlich startet es nicht mehr {#freecad_lief_normal_aber_plötzlich_startet_es_nicht_mehr}
...
-   Auf Linux: Navigiere zu {{FileName|/home/USERNAME/.FreeCAD}} und lösche dort die Dateien {{FileName|user.cfg}} und {{FileName|system.cfg}}
...
Es gibt ein [Makro findeKonfigDateien](Makro_findConfigFiles/de.md), das beim Auffinden deiner Konfigurationsdateien hilft. Es kann über den Erweiterungsverwalter im Menü Extras installiert werden. {{MenuCommand|Werkzeuge → Erweiterungsverwalter → Makros → findConfigFiles}}. Das Makro findet deinen Konfigurationsdateiordner, kopiert ihn in die Zwischenablage und (versucht), diesen Ort mit deinem Standard Dateibrowser zu öffnen. Es nimmt keine Änderungen an deinen Dateien oder Einstellungen vor.

## FreeCAD benutzen {#freecad_benutzen}

### Ist FreeCAD wirklich kostenlos? Sogar für kommerzielle Nutzung? {#ist_freecad_wirklich_kostenlos_sogar_für_kommerzielle_nutzung}
...
### Wie drehe ich die 3D Ansicht? {#wie_drehe_ich_die_3d_ansicht}


<center>

Image:Style\_of\_navigation.png\|Von der **right button** Maus Image:Style of navigation menu.png\|Vom Menü {{MenuCommand|Bearbeiten → Einstellungen →}}
...
### Was kann ich mit FreeCAD tun? Wo soll ich anfangen? {#was_kann_ich_mit_freecad_tun_wo_soll_ich_anfangen}
...
### Gibt es Dokumentation für Neueinsteiger? Wie kann ich die Benutzung von FreeCAD lernen? {#gibt_es_dokumentation_für_neueinsteiger_wie_kann_ich_die_benutzung_von_freecad_lernen}
...
### Ich möchte Daten im XYZ Format nach/von FreeCAD im-/exportieren. Wie mache ich das? {#ich_möchte_daten_im_xyz_format_nachvon_freecad_im_exportieren._wie_mache_ich_das}
...
## Arbeiten mit Part Geometrie {#arbeiten_mit_part_geometrie}

### Wie extrudiere ich Sachen in Festkörper? Ich erhalte nicht das richtige Ergebnis {#wie_extrudiere_ich_sachen_in_festkörper_ich_erhalte_nicht_das_richtige_ergebnis}
...
### Meine booleschen Operationen schlagen fehl oder liefern merkwürdige Ergebnisse {#meine_booleschen_operationen_schlagen_fehl_oder_liefern_merkwürdige_ergebnisse}
...
### Wenn ich mein Modell exportiere (oder anzeige), werden die Löcher ausgefüllt {#wenn_ich_mein_modell_exportiere_oder_anzeige_werden_die_löcher_ausgefüllt}
...
### Meine parametrischen Objekte gehen kaputt, wenn ich ihre Basisskizzen ändere {#meine_parametrischen_objekte_gehen_kaputt_wenn_ich_ihre_basisskizzen_ändere}
...
## Zu FreeCAD beitragen {#zu_freecad_beitragen}

### FreeCAD ist so ein tolles Programm! Wie kann ich helfen? {#freecad_ist_so_ein_tolles_programm_wie_kann_ich_helfen}
...
### Wie kann ich Bearbeitungsrechte für das Wiki erhalten? {#wie_kann_ich_bearbeitungsrechte_für_das_wiki_erhalten}
...
### Nimmt FreeCAD am \"Google Summer of Code\" teil? {#nimmt_freecad_am_google_summer_of_code_teil}
...
### Ich möchte anfangen, das Wiki in meine Sprache zu übersetzen. Was muss ich tun? {#ich_möchte_anfangen_das_wiki_in_meine_sprache_zu_übersetzen._was_muss_ich_tun}
...
## Lizenzierung, kopieren und Wiederverwendung {#lizenzierung_kopieren_und_wiederverwendung}

### Muss ich etwas bezahlen, um FreeCAD nutzen zu können? {#muss_ich_etwas_bezahlen_um_freecad_nutzen_zu_können}
...
### Kann ich einen beliebigen Teil der FreeCAD Illustrationen oder Teile der Webseite wiederverwenden? {#kann_ich_einen_beliebigen_teil_der_freecad_illustrationen_oder_teile_der_webseite_wiederverwenden}
...
### Kann ich Teile von FreeCAD in anderen Anwendungen wiederverwenden? {#kann_ich_teile_von_freecad_in_anderen_anwendungen_wiederverwenden}
