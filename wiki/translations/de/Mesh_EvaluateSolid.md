---
 GuiCommand:
   Name: Mesh EvaluateSolid
   MenuLocation: Meshes , Analyze , Check solid mesh
   Workbenches: Mesh_Workbench
   SeeAlso: Arch_SelectNonSolidMeshes
---

# Mesh EvaluateSolid/de



## Beschreibung

Der Befehl **Mesh EvaluateSolid** prüft, ob ein Netz für einen Festkörper geeignet ist. Solch ein Netzobjekt enthält keine Lücken.



## Anwendung

1.  Select a single mesh object.
2.  There are several ways to invoke the command:
    -   Press the **<img src="images/Mesh_EvaluateSolid.svg" width=16px> [Check solid mesh](Mesh_EvaluateSolid.md)** button.
    -   Select the **Meshes → Analyze → <img src="images/Mesh_EvaluateSolid.svg" width=16px> Check solid mesh** option from the menu.
3.  The **Solid Mesh** dialog box opens informing that the selected mesh object is solid or not.
4.  Press the **OK** button to close the dialog box and finish the command.





{{Mesh Tools navi

}}



---
⏵ [documentation index](../README.md) > [Mesh](Mesh_Workbench.md) > Mesh EvaluateSolid/de
