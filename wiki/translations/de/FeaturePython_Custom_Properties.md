...
ist die Art der Eigenschaft. Die unterschiedlichen Arten werden unten detaillierter beschrieben.
...
Ist die Art der Eigenschaft [App::PropertyEnumeration](#App:_PropertyEnumeration.md), besitzt die Set-Methode ein besonderes Verhalten: Das Setzen einer liste von Zeichenketten bestimmt die Auswahlmöglichkeiten der Aufzählung (enumeration); Das Setzen einer Zeichenkette wählt eine dieser Möglichkeiten aus. Um die Liste der Wahlmöglichkeiten zu erstellen und die aktuelle festzulegen, verwendet man:
...
Eine boolesche Eigenschaft. Sie kann `True` (wahr) und `False` (falsch) enthalten. Weitere Details finden sich im Abschnitt [Ein FeaturePython-Objekt erstellen und diesem eine Eigenschaft hinzufügen](#Ein_FeaturePython-Objekt_erstellen_und_diesem_eine_Eigenschaft_hinzufügen.md).
...
---
![](images/Button_right.svg) [documentation index](../README.md) > [API](Category_API.md) > [Developer Documentation](Category_Developer Documentation.md) > FeaturePython Custom Properties/de
